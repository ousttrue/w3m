...
bool HttpResponse::checkRedirection(const Url &pu) {
  ...
  for (auto &url : redirectins) {
    if (same_url_p(pu, url)) {
      // same url found !
      // auto tmp = Sprintf("Redirection loop detected (%s)",
      // pu.to_Str().c_str()); App::instance().disp_err_message(tmp->ptr);
      return false;
    }
  }
...
int HttpResponse::readHeader(const GetsFunc &gets, const Url &url) {
...
static void process_http_cookie(const Url *pu, std::string_view lineBuf2) {
  ...
  if (pu && name.size() > 0) {
    ...
    if (show_cookie) {
      if (flag & COO_SECURE) {
        // App::instance().disp_message_nsec("Received a secured cookie", 1,
        // true);
      } else {
        // App::instance().disp_message_nsec(
        //     Sprintf("Received cookie: %s=%s", name.c_str(),
        //     value.c_str())->ptr, 1, true);
      }
    }
...
void HttpResponse::pushHeader(const Url &url, std::string_view lineBuf2) {
  if ((url.scheme == SCM_HTTP || url.scheme == SCM_HTTPS) &&
      http_response_code == -1) {
    auto p = lineBuf2.begin();
    while (p != lineBuf2.end() && !IS_SPACE(*p))
      p++;
    while (p != lineBuf2.end() && IS_SPACE(*p))
      p++;
    http_response_code = atoi(std::string(p, lineBuf2.end()).c_str());
    // App::instance().message(lineBuf2->ptr);
  }

  if (!strncasecmp(lineBuf2.data(), "content-transfer-encoding:", 26)) {
    auto p = lineBuf2.begin() + 26;
    while (p != lineBuf2.end() && IS_SPACE(*p))
      p++;
  ...
  } else if (!strncasecmp(lineBuf2.data(), "content-encoding:", 17)) {
    // process_compression(lineBuf2, uf);
  } else if (use_cookie && accept_cookie &&
             check_cookie_accept_domain(url.host.c_str()) &&
             (!strncasecmp(lineBuf2.data(), "Set-Cookie:", 11) ||
              !strncasecmp(lineBuf2.data(), "Set-Cookie2:", 12))) {

    process_http_cookie(&url, lineBuf2);

  } else if (!strncasecmp(lineBuf2.data(), "w3m-control:", 12) &&
             url.scheme == SCM_LOCAL_CGI) {

    auto p = lineBuf2.begin() + 12;
    SKIP_BLANKS(p);
  ...
  auto view = remove_space(lineBuf2);
  document_header.push_back({view.begin(), view.end()});
}

std::string HttpResponse::getHeader(std::string_view field) const {
  if (field.empty()) {
    return {};
  }
  ...
  for (auto &i : this->document_header) {
    if (!strcasecmp(i.substr(0, len), field)) {
      auto p = i.c_str() + len;
      while (*p && IS_SPACE(*p)) {
        ++p;
      }
...
std::string HttpResponse::checkContentType() const {
  auto str = this->getHeader("Content-Type:");
  if (str.empty()) {
    return {};
  }
...
std::string HttpResponse::guess_save_name(std::string path) const {
  if (this->document_header.size()) {
    ...
    std::string p;
    const char *q;
    if ((p = this->getHeader("Content-Disposition:")).size() &&
        (q = strcasestr(p.c_str(), "filename")) &&
        (q == p || IS_SPACE(*(q - 1)) || *(q - 1) == ';') &&
        (name = matchattr(q, "filename"))) {
      path = *name;
    } else if ((p = this->getHeader("Content-Type:")).size() &&
               (q = strcasestr(p.c_str(), "name")) &&
               (q == p || IS_SPACE(*(q - 1)) || *(q - 1) == ';') &&
...
// void HttpResponse::page_loaded(Url url, const std::string &body) {
//   std::string p;
//   if ((p = this->getHeader("Content-Length:")).size()) {
//     this->current_content_length = strtoclen(p.c_str());
//   }
//
//   // if (do_download) {
//   //   /* download only */
...
CompressionType HttpResponse::contentEncoding() const {
  std::string p;
  if ((p = this->getHeader("Content-Encoding:")).size()) {
