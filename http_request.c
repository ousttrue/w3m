...
otherinfo(struct Url* target, struct Url* current, const char* referer)
{
    ...
    if (!getRuntime()->override_user_agent) {
        Strcat_charp(s, "User-Agent: ");
        if (url_user_agent)
            Strcat_charp(s, url_user_agent);
        else if (getRuntime()->UserAgent == NULL || *getRuntime()->UserAgent == '\0')
            Strcat_charp(s, w3m_version);
        else
            Strcat_charp(s, getRuntime()->UserAgent);
        Strcat_charp(s, "\r\n");
    }

    Strcat_m_charp(s, "Accept: ", getRuntime()->AcceptMedia, "\r\n", NULL);
    Strcat_m_charp(s, "Accept-Encoding: ", getRuntime()->AcceptEncoding, "\r\n", NULL);
    Strcat_m_charp(s, "Accept-Language: ", getRuntime()->AcceptLang, "\r\n", NULL);
    ...
    no_referer = getRuntime()->NoSendReferer;
    ...
    if (!no_referer) {
        ...
        if (getRuntime()->CrossOriginReferer && current && current->host && (!target || !target->host || strcasecmp(current->host, target->host) != 0 || current->port != target->port || current->scheme != target->scheme))
            cross_origin = TRUE;
        if (current && current->scheme == SCM_HTTPS && target->scheme != SCM_HTTPS) {
            /* Don't send Referer: if https:// -> http:// */
        } else if (referer == NULL && current && current->scheme != SCM_LOCAL && current->scheme != SCM_LOCAL_CGI && (current->scheme != SCM_FTP || (current->user == NULL && current->pass == NULL))) {
...
Str HTTPrequest(struct Url* pu, struct Url* current, struct HttpRequest* hr, struct TextList* extra)
{
    ...
    if (hr->command == HR_COMMAND_POST) {
        if (hr->request->enctype == FORM_ENCTYPE_MULTIPART) {
            ...
            if (!getRuntime()->override_content_type) {
                Strcat_charp(tmp,
            ...
            if (getRuntime()->header_string)
                Strcat(tmp, getRuntime()->header_string);
            Strcat_charp(tmp, "\r\n");
