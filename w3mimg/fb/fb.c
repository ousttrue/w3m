...
static int fb_video_adapter_info_get(int fbfp,
                                     video_adapter_info_t *video_adapter_info);
#endif
...
int fb_open(void) {
...
#elif defined(__FreeBSD__)
  if ((cmap = fb_cmap_create(&video_info, &video_adapter_info)) ==
  ...
  if (0) {
    ...
    pixel_size = 1;
           ...
           (vscinfo.bits_per_pixel == 15 || vscinfo.bits_per_pixel == 16 ||
...
int fb_image_draw(FB_IMAGE *image, int x, int y, int sx, int sy, int width,
  ...
  if (image == NULL || is_open != TRUE || sx > image->width ||
      sy > image->height || x > fb_width() || y > fb_height())
    return 1;
...
FB_IMAGE **fb_frame_new(int w, int h, int n) {
  FB_IMAGE **frame;
  int i, error = 0;
...
static int fb_get_cmap_index(int r, int g, int b) {
  ...
  if ((r & GREEN_MASK_8BIT) == (g & GREEN_MASK_8BIT) &&
    ...
    work = ((r & RED_MASK_8BIT) >> RED_SHIFT_8BIT) +
           ...
           ((b & BLUE_MASK_8BIT) >> BLUE_SHIFT_8BIT) + COLOR_OFFSET_8BIT;
  }
  return work;
}
...
static int fb_cmap_init(void) {
  ...
  if (cmap_org == NULL) {
#if defined(__linux__)
    if ((cmap_org = fb_cmap_create(&fscinfo, &vscinfo)) ==
        (struct fb_cmap *)-1) {
      return 1;
    }
#elif defined(__FreeBSD__)
    if ((cmap_org = fb_cmap_create(&video_info, &video_adapter_info)) ==
        (video_color_palette_t *)-1) {
      return 1;
    }
#endif
...
#endif
{
...
#if defined(__linux__)
  if ((buf =
           (unsigned char *)mmap(NULL, scinfo->smem_len, PROT_READ | PROT_WRITE,
                                 MAP_SHARED, fbfp, (off_t)0)) == MAP_FAILED) {
    perror("mmap error");
    return NULL;
  }
#elif defined(__FreeBSD__)
  ...
  mmap_length =
      (size_t)(video_adapter_info->va_window_size + mmap_offset + PAGE_MASK) &
      (~PAGE_MASK);
  if ((buf = (unsigned char *)mmap(NULL, mmap_length, PROT_READ | PROT_WRITE,
                                   MAP_SHARED, fbfp, (off_t)0)) == MAP_FAILED) {
    perror("mmap error");
    return NULL;
  }
