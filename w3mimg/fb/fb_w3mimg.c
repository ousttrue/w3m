...
static int w3mfb_init(w3mimg_op *self) {
  if (self == NULL)
    return 0;
  /* XXX */
  return 1;
...
static int w3mfb_finish(w3mimg_op *self) {
  if (self == NULL)
    return 0;
  return 1;
}

static int w3mfb_active(w3mimg_op *self) {
  if (self == NULL)
    return 0;
  return 1;
}

static void w3mfb_set_background(w3mimg_op *self, char *background) {
  if (self == NULL)
    return;
  if (background) {
    int r, g, b;
...
static void w3mfb_sync(w3mimg_op *self) { return; }

static void w3mfb_close(w3mimg_op *self) { fb_close(); }

static int w3mfb_clear(w3mimg_op *self, int x, int y, int w, int h) {
  if (self == NULL)
    return 0;
...
static int w3mfb_load_image(w3mimg_op *self, W3MImage *img, char *fname, int w,
                            int h) {
  FB_IMAGE **im;
...
static int w3mfb_show_image(w3mimg_op *self, W3MImage *img, int sx, int sy,
...
#define WAIT_CNT 4
  ...
  i = frame[0]->id;
  fb_image_draw(frame[i], x + self->offset_x, y + self->offset_y, sx, sy,
...
static void w3mfb_free_image(w3mimg_op *self, W3MImage *img) {
  if (self == NULL)
    return;
  if (img && img->pixmap) {
...
static int w3mfb_get_image_size(w3mimg_op *self, W3MImage *img, char *fname,
                                int *w, int *h) {
  int i;
...
static int check_tty_console(char *tty) {
  if (tty == NULL || *tty == '\0')
    return 0;
