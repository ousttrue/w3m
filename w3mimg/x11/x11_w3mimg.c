...
#if defined(USE_GTK2)
static int get_animation_size(GdkPixbufAnimation *animation, int *w, int *h,
                              int *delay) {
...
static int get_animation_size(GdkPixbufAnimation *animation, int *w, int *h,
  ...
  *h = gdk_pixbuf_animation_get_height(animation);
  for (i = 0; i < n; i++) {
    ...
    iw = gdk_pixbuf_frame_get_x_offset(frame) + gdk_pixbuf_get_width(pixbuf);
    ih = gdk_pixbuf_frame_get_y_offset(frame) + gdk_pixbuf_get_height(pixbuf);
    if (iw > *w)
      *w = iw;
    if (ih > *h)
      *h = ih;
...
static int x11_init(w3mimg_op *self) {
  struct x11_info *xi;
  if (self == NULL)
    return 0;
...
static int x11_finish(w3mimg_op *self) {
  struct x11_info *xi;
  if (self == NULL)
    return 0;
  xi = (struct x11_info *)self->priv;
...
static int x11_clear(w3mimg_op *self, int x, int y, int w, int h) {
  struct x11_info *xi;
  if (self == NULL)
    return 0;
  xi = (struct x11_info *)self->priv;
...
static int x11_active(w3mimg_op *self) {
  struct x11_info *xi;
  if (self == NULL)
    return 0;
  xi = (struct x11_info *)self->priv;
...
static void x11_set_background(w3mimg_op *self, char *background) {
  ...
  if (background && XAllocNamedColor(xi->display, attr.colormap, background,
...
static void x11_sync(w3mimg_op *self) {
  struct x11_info *xi;
  if (self == NULL)
    return;
  xi = (struct x11_info *)self->priv;
...
static void x11_close(w3mimg_op *self) { /* XCloseDisplay(xi->display); */ }

#if defined(USE_GDKPIXBUF)
static struct x11_image *x11_img_new(struct x11_info *xi, int w, int h, int n) {
  ...
  XGetWindowAttributes(xi->display, xi->window, &attr);
  for (i = 0; i < n; i++) {
    img->pixmap[i] = XCreatePixmap(xi->display, xi->parent, w, h, attr.depth);
    if (!img->pixmap[i])
      goto ERROR;

    XSetForeground(xi->display, xi->imageGC, xi->background_pixel);
    XFillRectangle(xi->display, (Pixmap)img->pixmap[i], xi->imageGC, 0, 0, w,
                   h);
  }
...
#if defined(USE_GTK2)
static void render_pixbuf_to_pixmap_32(Display *display, GC gc, Pixmap pixmap,
                                       GdkPixbuf *pixbuf) {
  ...
  if (!(image = XGetImage(display, pixmap, 0, 0, width, height, AllPlanes,
                          ZPixmap)))
    return;
...
static int x11_load_image(w3mimg_op *self, W3MImage *img, char *fname, int w,
                          int h) {
  struct x11_info *xi;
#if defined(USE_IMLIB)
  ImlibImage *im;
  ...
  if (h <= 0)
    h = im->rgb_height;
  img->pixmap =
      (void *)XCreatePixmap(xi->display, xi->parent, w, h, attr.depth);
  if (!img->pixmap)
    return 0;
  XSetForeground(xi->display, xi->imageGC, xi->background_pixel);
  ...
  im = imlib_create_cropped_scaled_image(0, 0, imlib_image_get_width(),
  ...
  tmp_pixmap = XCreatePixmap(xi->display, xi->parent, w, h, attr.depth);
  XSetForeground(xi->display, xi->imageGC, xi->background_pixel);
  ...
  for (j = 0; j < frame_num; j++) {
    ...
    if (attr.depth == 32)
      render_pixbuf_to_pixmap_32(xi->display, xi->imageGC, ximg->pixmap[j],
                                 pixbuf);
    else
      gdk_pixbuf_xlib_render_to_drawable_alpha(
          pixbuf, (Drawable)ximg->pixmap[j], 0, 0, 0, 0, w, h,
          GDK_PIXBUF_ALPHA_BILEVEL, 1, XLIB_RGB_DITHER_NORMAL, 0, 0);
  ...
  for (j = 0; j < n; j++) {
    ...
    XCopyArea(xi->display, tmp_pixmap, ximg->pixmap[i], xi->imageGC, 0, 0, w, h,
              0, 0);
    gdk_pixbuf_xlib_render_to_drawable_alpha(
        pixbuf, (Drawable)ximg->pixmap[i], 0, 0, ofstx, ofsty, width, height,
        GDK_PIXBUF_ALPHA_BILEVEL, 1, XLIB_RGB_DITHER_NORMAL, 0, 0);

    switch (gdk_pixbuf_frame_get_action(frame)) {
    case GDK_PIXBUF_FRAME_RETAIN:
      XCopyArea(xi->display, ximg->pixmap[i], tmp_pixmap, xi->imageGC, 0, 0, w,
                h, 0, 0);
      break;
    case GDK_PIXBUF_FRAME_DISPOSE:
      XSetForeground(xi->display, xi->imageGC, xi->background_pixel);
      XFillRectangle(xi->display, tmp_pixmap, xi->imageGC, 0, 0, w, h);
      break;
    case GDK_PIXBUF_FRAME_REVERT:
      XCopyArea(xi->display, ximg->pixmap[0], tmp_pixmap, xi->imageGC, 0, 0, w,
                h, 0, 0);
      break;
    default:
      XCopyArea(xi->display, ximg->pixmap[0], tmp_pixmap, xi->imageGC, 0, 0, w,
...
static int x11_show_image(w3mimg_op *self, W3MImage *img, int sx, int sy,
  ...
  XCopyArea(xi->display, (Pixmap)img->pixmap, xi->window, xi->imageGC, sx, sy,
            (sw ? sw : img->width), (sh ? sh : img->height), x + self->offset_x,
            y + self->offset_y);
#elif defined(USE_GDKPIXBUF)
#define WAIT_CNT 4
  if (ximg->delay <= 0)
    ...
    i = ximg->no;
  XCopyArea(xi->display, ximg->pixmap[i], xi->window, xi->imageGC, sx, sy,
            (sw ? sw : img->width), (sh ? sh : img->height), x + self->offset_x,
            y + self->offset_y);
  if (ximg->total > 1) {
...
static void x11_free_image(w3mimg_op *self, W3MImage *img) {
  struct x11_info *xi;
  if (self == NULL)
    return;
  xi = (struct x11_info *)self->priv;
...
static int x11_get_image_size(w3mimg_op *self, W3MImage *img, char *fname,
                              int *w, int *h) {
  struct x11_info *xi;
#if defined(USE_IMLIB)
  ImlibImage *im;
...
w3mimg_op *w3mimg_x11open() {
  ...
  while (1) {
    ...
    XQueryTree(xi->display, xi->window, &root, &xi->parent, &children,
               &nchildren);
    p_window = xi->window;
    for (i = 0; i < nchildren; i++) {
      XGetWindowAttributes(xi->display, children[i], &attr);
      if (attr.width > wop->width * 0.7 && attr.height > wop->height * 0.7) {
        /* maybe text window */
        wop->width = attr.width;
        wop->height = attr.height;
