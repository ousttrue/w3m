...
Str ssl_get_certificate(SSL* ssl, const char* hostname)
{
     ...
     */
    if (getRuntime()->ssl_verify_server) {
        long verr;
...
SSL* openSSLHandle(int sock, const char* hostname, const char** p_cert)
{
    ...
    if (getRuntime()->ssl_path_modified) {
        free_ssl_ctx();
    ...
    if (ssl_ctx == NULL) {
        ...
        if (getRuntime()->ssl_min_version && *getRuntime()->ssl_min_version != '\0') {
            int sslver;
            sslver = str_to_ssl_version(getRuntime()->ssl_min_version);
            if (sslver < 0
...
#endif
        if (getRuntime()->ssl_cipher && *getRuntime()->ssl_cipher != '\0')
            if (!SSL_CTX_set_cipher_list(ssl_ctx, getRuntime()->ssl_cipher)) {
                free_ssl_ctx();
                goto eend;
            }
        option = SSL_OP_ALL;
        if (getRuntime()->ssl_forbid_method) {
            if (strchr(getRuntime()->ssl_forbid_method, '2'))
                option |= SSL_OP_NO_SSLv2;
            if (strchr(getRuntime()->ssl_forbid_method, '3'))
                option |= SSL_OP_NO_SSLv3;
            if (strchr(getRuntime()->ssl_forbid_method, 't'))
                option |= SSL_OP_NO_TLSv1;
            if (strchr(getRuntime()->ssl_forbid_method, 'T'))
                option |= SSL_OP_NO_TLSv1;
            if (strchr(getRuntime()->ssl_forbid_method, '4'))
                option |= SSL_OP_NO_TLSv1;
#ifdef SSL_OP_NO_TLSv1_1
            if (strchr(getRuntime()->ssl_forbid_method, '5'))
                option |= SSL_OP_NO_TLSv1_1;
#endif
#ifdef SSL_OP_NO_TLSv1_2
            if (strchr(getRuntime()->ssl_forbid_method, '6'))
                option |= SSL_OP_NO_TLSv1_2;
#endif
#ifdef SSL_OP_NO_TLSv1_3
            if (strchr(getRuntime()->ssl_forbid_method, '7'))
        ...
        if (getRuntime()->ssl_cert_file != NULL && *getRuntime()->ssl_cert_file != '\0') {
            ...
            if (SSL_CTX_use_certificate_file(ssl_ctx, getRuntime()->ssl_cert_file, SSL_FILETYPE_PEM) > 0) {
                char* key_file = (getRuntime()->ssl_key_file == NULL
                                     || *getRuntime()->ssl_key_file == '\0')
                    ? getRuntime()->ssl_cert_file
                    : getRuntime()->ssl_key_file;
        ...
        if (getRuntime()->ssl_verify_server) {
            ...
            if (getRuntime()->ssl_ca_file && *getRuntime()->ssl_ca_file != '\0')
                file = getRuntime()->ssl_ca_file;
            if (getRuntime()->ssl_ca_path && *getRuntime()->ssl_ca_path != '\0')
                path = getRuntime()->ssl_ca_path;
            if ((file || path)
