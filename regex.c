...
newRegex0(char** ex, int igncase, Regex* regex, char** msg, int level)
{
    ...
    for (p = *ex; *p != '\0'; p++) {
        ...
        case '^':
            ...
            break;
        case '+':
            if (re == regex->re || (RE_MODE(re - 1) != RE_ANY && (re - 1)->p.pattern == NULL)) {
                if (msg)
    ...
    char* str, char* end_p, int iter_limit, int firstp)
{
    ...
    c->ctx = malloc(sizeof(struct MatchingContext1));
    c->ctx2 = malloc(sizeof(struct MatchingContext2));
    c->ctx->label = 0;
    c->regex = regex;
    c->n_any = 0;
    c->str = str;
    ...
    regexchar* re, char* str, char* end_p, int firstp)
{
    ...
    while (RE_MODE(c->re) != RE_ENDMARK) {
        ...
        case RE_SUBREGEX:
            if (c->sub_ctx == NULL) {
                c->sub_ctx = malloc(sizeof(struct MatchingContext1));
            }
