...
static Regex *newRegex0(char **ex, int igncase, Regex *regex, char **msg,
  ...
  for (p = *ex; *p != '\0'; p++) {
    ...
    if (st_ptr >= &regex->storage[STORAGE_MAX] || re >= &regex->re[REGEX_MAX]) {
      if (msg)
        *msg = "Regular expression too long";
      return NULL;
    }
  ...
  for (;;) {
    ...
    while (regmatch_iter(c->ctx, c->regex->re, c->str, end_p, c->firstp)) {
      ...
      if (RE_MODE(pat2) == RE_ENDMARK) {
        ...
        YIELD(1, c, 1);
      } else if (regmatch(pat2, c->str + c->n_any, end_p, c->firstp,
                          &c->lastpos) == 1) {
        YIELD(1, c, 2);
      }
      if (iter_limit == 1)
        continue;
      c->ctx2->label = 0;
      while (regmatch_sub_anytime(c->ctx2, regex, pat2, c->str + c->n_any,
                                  end_p, iter_limit - 1, c->firstp)) {
...
static int regmatch_iter(struct MatchingContext1 *c, regexchar *re, char *str,
  ...
  while (RE_MODE(c->re) != RE_ENDMARK) {
      ...
      while (c->n_any < c->iter_limit) {
        ...
        if (c->n_any >= 0) {
          if (RE_MODE(c->re) == RE_SUBREGEX) {
            c->ctx2 =
                (MatchingContext2 *)GC_malloc(sizeof(struct MatchingContext2));
            c->ctx2->label = 0;
        ...
        if (RE_MODE(c->re + 1) == RE_ENDMARK) {
          c->lastpos = c->str + c->n_any;
          YIELD(1, c, 2);
        } else if (regmatch(c->re + 1, c->str + c->n_any, c->end_p, c->firstp,
                            &c->lastpos) == 1) {
          YIELD(1, c, 3);
        }
      }
    ...
    case RE_SUBREGEX:
      if (c->sub_ctx == NULL) {
        c->sub_ctx =
            (MatchingContext1 *)GC_malloc(sizeof(struct MatchingContext1));
      }
      c->sub_regex = c->re->p.sub;
      for (;;) {
        c->sub_ctx->label = 0;
        while (regmatch_iter(c->sub_ctx, c->sub_regex->re, c->str, c->end_p,
                             c->firstp)) {
...
static int regmatch1(regexchar *re, longchar *c) {
  int ans;
...
#ifdef REGEX_DEBUG
    if (verbose)
      printf("RE=%s vs %s -> %d\n", lc2c(re->p.pattern, 1), lc2c(c, 1), ans);
#endif /* REGEX_DEBUG */
    return ans;
  case RE_WHICH:
...
static int matchWhich(longchar *pattern, longchar *c, int igncase) {
  longchar *p = pattern;
  int ans = 0;
...
static int match_longchar(longchar *a, longchar *b, int ignore) {
  if (a->type != b->type)
    return 0;
  if (a->type == RE_TYPE_WCHAR_T) {
    if (ignore) {
      wc_uint32 ua = wc_any_to_ucs(a->wch), ub = wc_any_to_ucs(b->wch);
      return (ua == ub || ua == wc_ucs_tolower(ub) ||
...
static int match_range_longchar(longchar *a, longchar *b, longchar *c,
                                int ignore) {
  ...
  if (a->type == RE_TYPE_WCHAR_T) {
    ...
    if (ignore) {
      ...
      if (wc_is_ucs_alpha(uc)) {
        ...
        return ((ua <= upper && upper <= ub) || (ua <= lower && lower <= ub) ||
                (ua <= title && title <= ub));
      }
    }
