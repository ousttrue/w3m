...
static struct frame_body *copyFrame(struct frame_body *ob) {
  ...
  memcpy((void *)rb, (const void *)ob, sizeof(struct frame_body));
  return rb;
}
...
struct frameset *copyFrameSet(struct frameset *of) {
  ...
  n = of->col * of->row;
  memcpy((void *)rf, (const void *)of, sizeof(struct frameset));
          ...
          if (is_tag) {
            ...
            for (j = 0; j < TagMAP[tag->tagid].max_attribute; j++) {
              ...
              case ATTR_ACTION:
                ...
                if (url.scheme == SCM_UNKNOWN || url.scheme == SCM_MAILTO ||
