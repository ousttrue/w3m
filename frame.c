    ...
    struct Url* baseURL, int flag)
{
    ...
    default:
        ...
        buf = loadGeneralFile(b->url,
            ...
            (struct LoadOption) { .referer = b->referer, .flag = flag | RG_FRAME_SRC }, b->request, false);
        /* XXX certificate? */
        if (buf)
                ...
                do {
                    ...
                    if (is_tag) {
                        if (pre_mode & (RB_PLAIN | RB_INTXTA | RB_SCRIPT | RB_STYLE)) {
                            ...
                            if (strncmp(tok->ptr, "<!--", 4) && (q = strchr(tok->ptr + 1, '<'))) {
...
renderFrame(struct Buffer* Cbuf, int force_reload)
{
    ...
    struct Buffer* buf = loadGeneralFile(tmp->ptr, NULL, (struct LoadOption) { .referer = NULL, .flag = flag }, NULL, false);
    ...
    for (i = 0; i < fset->col * fset->row; i++) {
        ...
        if (e->element != NULL) {
            if (e->element->name && !strcmp(e->element->name, name)) {
                return e;
            } else if (e->element->attr == F_FRAMESET && (e = search_frame(e->set, name))) {
                return e;
            }
        }
    }
