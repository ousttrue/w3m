...
pub fn extractIncludeDirsFromCompileStep(b: *std.Build, step: *std.Build.Step.Compile) []const []const u8 {
    var dirs = std.array_list.Managed(std.Build.LazyPath).init(b.allocator);
    defer dirs.deinit();

    // populates dirs
    extractIncludeDirsFromCompileStepInner(b, step, &dirs);

    var dirs_as_strings = std.array_list.Managed([]const u8).init(b.allocator);
...
fn makeCSourcePathsAbsolute(b: *std.Build, c_sources: CSourceFiles) AbsoluteCSourceFiles {
    var cpaths = std.array_list.Managed([]const u8).init(b.allocator);
    defer cpaths.deinit();
...
fn getCSources(b: *std.Build, steps: []const *std.Build.Step.Compile) []*AbsoluteCSourceFiles {
    ...
    var res = std.array_list.Managed(*AbsoluteCSourceFiles).init(allocator);

    // move the compile steps into a mutable dynamic array, so we can add
    // any child steps
    var compile_steps_list = std.array_list.Managed(*std.Build.Step.Compile).init(b.allocator);
    ...
    while (index < compile_steps_list.items.len) {
        ...
        var shared_flags = std.array_list.Managed([]const u8).init(allocator);
...
fn makeCdb(step: *std.Build.Step, make_options: std.Build.Step.MakeOptions) anyerror!void {
    ...
    var compile_commands = std.array_list.Managed(CompileCommandEntry).init(allocator);
