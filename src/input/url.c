...
struct Url parseURL2(const char *url, struct Url *current) {
  auto pu = parseURL(url, current);
  ...
  if (pu.scheme == SCM_NEWS || pu.scheme == SCM_NEWS_GROUP) {
    ...
    if (pu.file && !strchr(pu.file, '@') &&
        (!(p = strchr(pu.file, '/')) || strchr(p + 1, '-') || *(p + 1) == '\0'))
  ...
  if (current &&
    ...
    pu.port = current->port;
    if (pu.file && *pu.file) {
      if (pu.file[0] != '/' && !(pu.scheme == SCM_LOCAL &&
  ...
  if (pu.file) {
    if (pu.scheme == SCM_LOCAL && pu.file[0] != '/' &&
        !(IS_ALPHA(pu.file[0]) && pu.file[1] == ':') && strcmp(pu.file, "-")) {
    ...
    if (pu.scheme == SCM_LOCAL) {
      ...
      if (pu.host && !is_localhost(pu.host)) {
        ...
        Strcat_m_charp(tmp, pu.host, cleanupName(file_unquote(pu.file)), NULL);
...
struct Url parseURL(const char *_url, struct Url *current) {
  // const char *q, *qq;
  // Str tmp;
...
analyze_file:
  ...
  if (p_url.scheme == SCM_LOCAL && p_url.user == NULL && p_url.host != NULL &&
      *p_url.host != '\0' && !is_localhost(p_url.host)) {
    /*
