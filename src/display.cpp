...
#define define_effect(name_start, name_end, color_start, color_end, \
                      mono_start, mono_end)                         \
...
define_effect(EFFECT_ANCHOR_START, EFFECT_ANCHOR_END, EFFECT_ANCHOR_START_C,
    ...
    define_effect(EFFECT_IMAGE_START, EFFECT_IMAGE_END, EFFECT_IMAGE_START_C,
        ...
        define_effect(EFFECT_FORM_START, EFFECT_FORM_END, EFFECT_FORM_START_C,
            ...
            define_effect(EFFECT_MARK_START, EFFECT_MARK_END,
...
void displayBuffer(Buffer *buf, int mode)
{
    ...
    {
        if (!(activeImage && displayImage && draw_image_flag) && mode == B_SCROLL &&
            cline && buf->currentColumn == ccolumn)
        {
        ...
        {
            if (activeImage && (mode == B_REDRAW_IMAGE || cline != buf->topLine ||
...
static Line *redrawLineImage(Buffer *buf, Line *l, int i)
{
    ...
    for (j = 0; rcol - column < buf->COLS && pos + j < l->len; j++)
    {
        ...
        if (a && a->image && a->image->touch < image_touch)
        {
            ...
            cache = image->cache = getImage(image, baseURL(buf), buf->image_flag);
            if (cache)
            {
...
void addMChar(char *p, Lineprop mode, size_t len)
{
    ...
    {
        char buf[5];
        sprintf(buf, "[%.2X]", (unsigned char)wtf_get_code((wc_uchar *)p) | 0x80);
        addstr(buf);
    }
    else
        addmch(p, len);
...
Buffer *message_list_panel(void)
{
    ...
    if (message_list)
        for (p = message_list->last; p; p = p->prev)
            Strcat_m_charp(tmp, "<tr><td><pre>", (char *)html_quote((char *)p->ptr),
                           "</pre></td></tr>\n", NULL);
    else
                       ...
                       int mouse)
{
    ...
    refresh();
    tty::sleep_till_anykey(sec, purge);
