...
static Str ftp_command(FTP ftp, char *cmd, char *arg, int *status) {
  ...
  *status = -1; /* error */
  ...
  if (IS_DIGIT(tmp->ptr[0]) && IS_DIGIT(tmp->ptr[1]) && IS_DIGIT(tmp->ptr[2]) &&
      tmp->ptr[3] == ' ')
  ...
  if (ftppass_hostnamegen && !strcmp(ftp->user, "anonymous")) {
    ...
    if (n > 0 && ftp->pass[n - 1] == '@') {
      ...
      if (!getsockname(sock, (struct sockaddr *)&sockname, &socknamelen)) {
        ...
        if (getnameinfo((struct sockaddr *)&sockname, socknamelen, hostbuf,
                        sizeof hostbuf, NULL, 0, NI_NAMEREQD) == 0)
          Strcat_charp(tmp, hostbuf);
        else if (getnameinfo((struct sockaddr *)&sockname, socknamelen, hostbuf,
        ...
        if ((sockent =
                 gethostbyaddr((char *)&sockname.sin_addr,
                               sizeof(sockname.sin_addr), sockname.sin_family)))
          Strcat_charp(tmp, sockent->h_name);
        else
          Strcat_m_charp(tmp, "[", inet_ntoa(sockname.sin_addr), "]", NULL);
#endif
        ftp->pass = tmp->ptr;
      }
    }
...
static int ftp_pasv(FTP ftp) {
  ...
  if (getpeername(fileno(ftp->wf), (struct sockaddr *)&sockaddr, &sockaddrlen) <
      0)
    return -1;
  ...
  case AF_INET6:
    ...
    if (*p == '\0')
      return -1;
    if (sscanf(++p, "%c%c%c%d%c", &d1, &d2, &d3, &port, &d4) != 5 || d1 != d2 ||
        d1 != d3 || d1 != d4)
      return -1;
...
static time_t ftp_modtime(FTP ftp, char *path) {
  ...
  if (sscanf(p, "%04d%02d%02d%02d%02d%02d", &tm.tm_year, &tm.tm_mon,
...
static void closeFTPdata(FILE *f) {
  int status;
  if (f) {
    fclose(f);
...
InputStream openFTPStream(ParsedURL *pu, URLFile *uf) {
  ...
  if (current_ftp.host) {
    if (!strcmp(current_ftp.host, pu->host) && current_ftp.port == pu->port &&
...
ftp_read:
  ...
  if (status == 125 || status == 150)
    return newFileStream(current_ftp.data, (void (*)(void*))closeFTPdata);
...
void disconnectFTP(void) { ftp_quit(&current_ftp); }
