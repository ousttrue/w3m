...
void proc_wrapped_line(struct LineProcStatus *st, struct Url currentURL, Str line) {
  int pos = 0;
  ...
  while (str < endp) {
    ...
    if ((st->effect | ex_efct(st->ex_effect)) & PC_SYMBOL && *str != '<') {
      ...
      PPUSH(st, &pos, mode | st->effect | ex_efct(st->ex_effect), *(str++));
    } else if (*str == '&') {
      /*
       * & escape processing
       */
      ...
      case HTML_INPUT_ALT: {
        ...
        if (hseq > 0) {
          ...
          if (*str == '[')
            hpos++;
          st->doc->hmarklist = putHmarker(st->doc->hmarklist,
                                          currentLn(st->doc), hpos, hseq - 1);
        } else if (hseq < 0) {
          int h = -hseq - 1;
          int hpos = pos;
          if (*str == '[')
...
struct Document *render_to_lines(int cols, struct Url currentURL,
  ...
  if (out_size) {
    ...
    st.out_size = LINELEN;
    st.outc = NewAtom_N(char, out_size);
    st.outp = NewAtom_N(Lineprop, out_size);
  }
