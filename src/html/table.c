...
static double weight2(int a) { return (double)a / term_size().cols * 4 + 1.; }
      ...
      if (rowspan > 1) {
        ...
        if (c >= cell.size) {
          if (cell.size == 0) {
            ...
            cell.rowspan = New_Reuse(short, cell.rowspan, cell.size);
            cell.indexarray = New_Reuse(short, cell.indexarray, cell.size);
...
int get_table_width(struct table *t, short *orgwidth, short *cellwidth,
                    int flag) {
#ifdef __GNUC__
...
#define MAX_TABWIDTH 10000
  ...
  for (i = 0; i <= t->maxcol; i++) {
    for (j = 0; j <= t->maxrow; j++) {
      ...
      t->tabdata[j][i] =
          appendGeneralList((struct GeneralList *)l, t->tabdata[j][i]);
    }
  }
