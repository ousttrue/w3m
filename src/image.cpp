...
int getCharSize()
{
    ...
    set_environ("W3M_TTY", tty::get_name());

    if (enable_inline_image)
    {
        int ppc, ppl;

        if (tty::get_pixel_per_cell(&ppc, &ppl))
        {
...
void addImage(ImageCache *cache, int x, int y, int sx, int sy, int w, int h)
{
    ...
    if (n_terminal_image >= max_terminal_image)
    {
        ...
        terminal_image =
            New_Reuse(TerminalImage, terminal_image, max_terminal_image);
    }
    ...
    for (j = 0; j < n_terminal_image; j++)
    {
        ...
        if (enable_inline_image)
        {
            ...
            int w = i->cache->a_width > 0
                        ? ((i->cache->width + i->x % pixel_per_char_i +
                            pixel_per_char_i - 1) /
                           pixel_per_char_i)
                        : 0;
            ...
            else if (enable_inline_image == INLINE_IMG_KITTY)
            {
                put_image_kitty(url, x, y, i->width, i->height, i->sx, i->sy,
                                sw * pixel_per_char, sh * pixel_per_line_i, sw, sh);
            }
        ...
        sprintf(buf, "%d;%d;%d;%d;%d;%d;%d;%d;%d;",
                ...
                (i->cache->height > 0) ? i->cache->height : 0, i->sx, i->sy,
                i->width, i->height);
...
static int parseImageHeader(char *path, u_int *width, u_int *height)
{
    FILE *fp;
    u_char buf[8];
...
int getImageSize(ImageCache *cache)
{
    ...
    if (parseImageHeader(cache->file, (u_int *)&w, (u_int *)&h))
        goto got_image_size;
