...
disp_srchresult(int result, char* prompt, char* str)
{
    ...
    if (result & SR_NOTFOUND)
        message(MSG_INFO, Sprintf("Not found: %s", str)->ptr);
    else if (result & SR_WRAPPED)
        message(MSG_INFO, Sprintf("Search wrapped: %s", str)->ptr);
    else if (show_srch_str)
        message(MSG_INFO, Sprintf("%s%s", prompt, str)->ptr);
}
...
DEFUN(pipeBuf, PIPE_BUF, "Pipe current buffer through a shell command and display output")
{
    ...
    if (f == NULL) {
        /* FIXME: gettextize? */
        message(MSG_INFO, Sprintf("Can't save buffer to %s", cmd)->ptr);
        return;
    }
    ...
    if (buf == NULL) {
        message(MSG_INFO, "Execution failed");
        return;
    } else {
        buf->filename = cmd;
...
DEFUN(editBf, EDIT, "Edit local source")
{
    ...
    if (fn == NULL || Currentbuf->pagerSource != NULL || /* Behaving as a pager */
        ...
        message(MSG_ERR, "Can't edit other than local file");
        return;
    }
    if (Currentbuf->edit)
...
DEFUN(editScr, EDIT_SCREEN, "Edit rendered copy of document")
{
    ...
    if (f == NULL) {
        /* FIXME: gettextize? */
        message(MSG_ERR, Sprintf("Can't open %s", tmpf)->ptr);
        return;
    }
    saveBuffer(Currentbuf, f, TRUE);
    fclose(f);
...
DEFUN(nextMk, NEXT_MARK, "Go to the next mark")
{
    ...
    /* FIXME: gettextize? */
    message(MSG_INFO, "No mark exist after here");
}
...
DEFUN(prevMk, PREV_MARK, "Go to the previous mark")
{
    ...
    /* FIXME: gettextize? */
    message(MSG_INFO, "No mark exist before here");
}
...
loadLink(char* url, char* target, char* referer, FormList* request)
{
    ...
    message(MSG_INFO, Sprintf("loading %s", url)->ptr);
    // refresh(ttyWriter());
...
gotoLabel(char* label)
{
    ...
    if (al == NULL) {
        /* FIXME: gettextize? */
        message(MSG_INFO, Sprintf("%s is not found", label)->ptr);
        return;
    }
...
handleMailto(char* url)
{
    ...
    if (!non_null(Mailer)) {
        /* FIXME: gettextize? */
        message(MSG_ERR, "no mailer is specified");
        return 1;
    }
#endif
...
DEFUN(followI, VIEW_IMAGE, "Display image in viewer")
{
    ...
    if (a == NULL)
        return;
    /* FIXME: gettextize? */
    message(MSG_INFO, Sprintf("loading %s", a->url)->ptr);
    // refresh(ttyWriter());
...
_followForm(int submit)
{
    ...
    case FORM_INPUT_TEXT:
        ...
        if (fi->readonly)
            /* FIXME: gettextize? */
            message(MSG_INFO, "Read only field!");
        /* FIXME: gettextize? */
    ...
    do_submit:
        ...
        if (fi->parent->method == FORM_METHOD_GET) {
            ...
            message(MSG_ERR, "Can't send form because of illegal method.");
        }
        break;
    case FORM_INPUT_RESET:
...
DEFUN(backBf, BACK, "Close current buffer and return to the one below in stack")
{
    if (!checkBackBuffer(Currentbuf)) {
        /* FIXME: gettextize? */
        message(MSG_INFO, "Can't go back...");
        return;
    }
...
{
    if (buf == NULL) {
        message(MSG_ERR, "Can't load string");
    } else if (buf != NO_BUFFER) {
...
DEFUN(extbrz, EXTERN, "Display using an external browser")
{
    if (Currentbuf->bufferprop & BP_INTERNAL) {
        /* FIXME: gettextize? */
        message(MSG_ERR, "Can't browse...");
        return;
    }
    if (Currentbuf->currentURL.scheme == SCM_LOCAL && !strcmp(Currentbuf->currentURL.file, "-")) {
        /* file is std input */
        /* FIXME: gettextize? */
        message(MSG_ERR, "Can't browse stdin");
        return;
    }
...
{
    message(MSG_INFO, Sprintf("w3m version %s", w3m_version)->ptr);
}
...
DEFUN(wrapToggle, WRAP_TOGGLE, "Toggle wrapping mode in searches")
{
    if (WrapSearch) {
        WrapSearch = FALSE;
        /* FIXME: gettextize? */
        message(MSG_INFO, "Wrap search off");
    } else {
        WrapSearch = TRUE;
        /* FIXME: gettextize? */
        message(MSG_INFO, "Wrap search on");
    }
}
...
DEFUN(setAlarm, ALARM, "Set alarm")
{
    ...
    if (cmd >= 0) {
        ...
        message(MSG_INFO, Sprintf("%dsec %s %s", sec, w3mFuncList[cmd].id, data) ->ptr);
    } else {
...
DEFUN(reinit, REINIT, "Reload configuration file")
{
    ...
    message(MSG_ERR, Sprintf("Don't know how to reinitialize '%s'", resource)->ptr);
}
