...
void set_table_matrix(struct table*, int, int);

static double
weight(int x, int cols)
{
...
static double
weight2(int a, int cols)
{
    return (double)a / cols * 4 + 1.;
}

static double sigma_td(int a, int cols) { return (0.5 * weight2(a, cols)); } /* <td width=...> */
static double sigma_td_nw(int a, int cols) { return (32 * weight2(a, cols)); } /* <td ...> */
static double sigma_table(int a, int cols) { return (0.25 * weight2(a, cols)); } /* <table width=...> */
static double sigma_table_nw(int a, int cols) { return (2 * weight2(a, cols)); } /* <table...> */
...
table_rowspan(struct table* t, int row, int col)
{
    ...
    if (!t->tabattr[row])
        return 0;
    for (i = row + 1; i <= t->maxrow && t->tabattr[i] && (t->tabattr[i][col] & HTT_Y); i++)
        ;
    return i - row;
}
...
int visible_length(const char* str)
{
    ...
    enum ReadtokenStatus prev_status = status;
    Str tagbuf = Strnew();
    const char *t, *r2;
    int amp_len = 0;
...
void do_refill(struct table* tbl, int row, int col, int maxlimit, int cols)
{
    ...
    init_henv(&h_env, &obuf, envs, MAX_ENV_LEVEL,
        ...
        get_spec_cell_width(tbl, row, col), 0, cols);
    ...
    for (step = 0; step < 2; step++) {
        for (i = 0; i <= t->maxcol; i += n) {
            ...
            for (k = 0; k < n; k++) {
                ...
                if (fixed[ii] < 2 && iwidth[ii] - rulewidth < t->minimum_width[ii])
                    fixed[ii] = 2;
                if (fixed[ii] < 1 && iwidth[ii] - rulewidth < t->tabwidth[ii] && (double)rulewidth - mod[ii] > 0.5)
                    fixed[ii] = 1;
            }
            ...
            for (k = 0; k < cell->maxcell; k++) {
                ...
                w = 0;
                for (kk = 0; kk < m; kk++) {
                    if (fixed[(int)idx[kk]] < 1 && (double)rulewidth - mod[(int)idx[kk]] > 0.5)
                        w += rulewidth;
                }
...
_end:
    if (dmax > 0. && dmin > dmax)
        dmin = dmax;
    span = ecol - bcol;
    if ((span == t->maxcol + 1 && dmin >= 0.) || (span != t->maxcol + 1 && dmin > rulewidth * 0.5)) {
...
check_table_width(struct table* t, double* newwidth, MAT* minv, int itr)
{
    ...
    for (i = 0; i <= t->maxcol; i++) {
        ...
        if (sx < 0.1)
            continue;
        if (orgwidth[i] < t->minimum_width[i] && corwidth[i] == t->minimum_width[i]) {
...
static void
renderCoTable(struct table* tbl, int maxlimit, int cols)
{
    struct readbuffer obuf;
    struct html_feed_environ h_env;
    struct environment envs[MAX_ENV_LEVEL];
    ...
    for (i = 0; i < tbl->ntable; i++) {
        ...
        init_henv(&h_env, &obuf, envs, MAX_ENV_LEVEL, tbl->tables[i].buf,
            get_spec_cell_width(tbl, row, col), indent, cols);
        check_row(tbl, row);
        if (h_env.limit > maxlimit)
            h_env.limit = maxlimit;
        if (t->total_width == 0)
...
make_caption(struct table* t, struct html_feed_environ* h_env)
{
    ...
    init_henv(&henv, &obuf, envs, MAX_ENV_LEVEL, newTextLineList(),
        limit, h_env->envs[h_env->envc].indent, h_env->cols);
...
#define MAX_TABWIDTH 10000
    ...
    if (t->maxcol == 0) {
        ...
        set_table_matrix(t, max_width, h_env->cols);
    ...
    renderCoTable(t, h_env->limit, h_env->cols);
    ...
    for (i = 0; i <= t->maxcol; i++) {
        for (j = 0; j <= t->maxrow; j++) {
            ...
            if (t->tabattr[j][i] & HTT_Y)
                continue;
            do_refill(t, j, i, h_env->limit, h_env->cols);
        }
    }
...
void check_rowcol(struct table* tbl, struct table_mode* mode)
{
    ...
    for (;; tbl->row++) {
        ...
        for (; tbl->col < MAXCOL && tbl->tabattr[tbl->row][tbl->col] & (HTT_X | HTT_Y); tbl->col++)
            ;
        if (tbl->col < MAXCOL)
            break;
        tbl->col = 0;
    ...
    int checkminimum)
{
    ...
    while (*line) {
        ...
        if (ctype == PC_ASCII && IS_SPACE(*c)) {
            ...
            if (prevchar->length && is_boundary((unsigned char*)prevchar->ptr, (unsigned char*)c)) {
                w = len;
            } else {
                w += len;
            }
...
table_close_anchor0(struct table* tbl, struct table_mode* mode)
{
    ...
    if (tbl->tabcontentssize == mode->anchor_offset) {
        ...
        setwidth(tbl, mode);
    } else if (tbl->linfo.prev_spaces > 0 && tbl->tabcontentssize - 1 == mode->anchor_offset) {
    ...
    int width, struct HtmlTagParsed* tag)
{
        ...
        for (i = 0; i < rowspan; i++) {
            ...
            for (j = 0; j < colspan; j++) {
                if (!(tbl->tabattr[tbl->row + i][tbl->col + j] & (HTT_X | HTT_Y))) {
                    tbl->tabattr[tbl->row + i][tbl->col + j] |= ((i > 0) ? HTT_Y : 0) | ((j > 0) ? HTT_X : 0);
                }
    ...
    case HTML_TEXTAREA:
        w = 0;
        check_rowcol(tbl, mode);
        if (tbl->col + 1 <= tbl->maxcol && tbl->tabattr[tbl->row][tbl->col + 1] & HTT_X) {
    ...
    int width, int internal)
{
    ...
    if (*line == '<' && line[1] && REALLY_THE_BEGINNING_OF_A_TAG(line)) {
        ...
        if (tag) {
            ...
            if (!(mode->pre_mode & (TBLM_PLAIN | TBLM_INTXTA | TBLM_INSELECT | TBLM_SCRIPT | TBLM_STYLE)))
                return -1;
        }
    } else {
        if (mode->pre_mode & (TBLM_DEL | TBLM_S))
    ...
    if (!(mode->pre_mode & (TBLM_SPECIAL & ~TBLM_NOBR))) {
        ...
        int i = skip_space(tbl, line, linfo, !(mode->pre_mode & TBLM_NOBR));
        ...
        while (*line) {
            ...
            if ((p = strchr(line, '\r')) || (p = strchr(line, '\n'))) {
                ...
                if (p[1]) {
                    p++;
                    Str tmp = Strnew_charp_n(line, p - line);
                    line = p;
                    p = tmp->ptr;
                } else {
                    p = line;
...
static void
set_table_matrix0(struct table* t, int maxwidth, int cols)
{
    ...
    if (cell->necell == 0) {
        for (i = 0; i < size; i++) {
            s = we[i] / w0;
            b = sigma_td_nw((int)(s * maxwidth), cols);
    ...
    for (k = 0; k < cell->necell; k++) {
        ...
        for (i = bcol; i < ecol; i++) {
            w = weight(width * (t->tabwidth[i] + 0.1) / w1, cols);
            if (w > we[i])
                we[i] = w;
        }
    }
...
void set_table_matrix(struct table* t, int width, int cols)
{
    int size = t->maxcol + 1;
    int i, j;
    double b, s;
    ...
    set_table_matrix0(t, width, cols);

    if (t->total_width > 0) {
        b = sigma_table(width, cols);
    } else {
        b = sigma_table_nw(width, cols);
    }
