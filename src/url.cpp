...
void parseURL2(char *url, ParsedURL *pu, ParsedURL *current)
{
        ...
        pu->host == NULL)
    {
        ...
        if (pu->file && *pu->file)
        {
            if (pu->scheme == SCM_UNKNOWN && strchr(pu->file, ':') == NULL &&
                current && (p = strchr(current->file, ':')) != NULL)
            {
                pu->file = Sprintf("%s:%s", allocStr(current->file, p - current->file),
                                   pu->file)
                               ->ptr;
            }
            else if (pu->file[0] != '/'
...
static struct table2 default_urimethods[] = {
    {"mailto", "file:///$LIB/w3mmail.cgi?%s"}, {NULL, NULL}};

static struct table2 *loadURIMethods(char *filename)
{
    FILE *f;
    int i, n;
    Str tmp;
...
wc_ces url_to_charset(const char *url, const ParsedURL *base,
                      wc_ces doc_charset)
{
    const ParsedURL *pu;
    ParsedURL pu_buf;
...
{
    return url_quote_conv((char *)url, url_to_charset(url, base, doc_charset));
}
...
char *url_decode2(const char *url, const Buffer *buf)
{
    ...
    if (!DecodeURL)
        return (char *)url;
    url_charset =
        buf ? url_to_charset(url, baseURL((Buffer *)buf), buf->document_charset)
