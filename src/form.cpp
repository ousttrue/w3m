...
struct form_item_list *formList_addInput(struct form_list *fl,
  ...
  if (parsedtag_get_value(tag, ATTR_SELECTNUMBER, &i) && i >= 0 &&
      i < max_select)
...
static struct pre_form *add_pre_form(struct pre_form *prev, char *url,
  ...
  ParsedURL pu;
  struct pre_form *new_form;

  if (prev)
    new_form = prev->next = New(struct pre_form);
  else
    new_form = PreForm = New(struct pre_form);
  if (url && !re_url) {
    parseURL2(url, &pu, NULL);
    new_form->url = parsedURL2Str(&pu)->ptr;
  } else
    new_form->url = url;
  new_form->re_url = re_url;
  new_form->name = (name && *name) ? name : NULL;
  new_form->action = (action && *action) ? action : NULL;
  new_form->item = NULL;
  new_form->next = NULL;
  return new_form;
}
...
static struct pre_form_item *add_pre_form_item(struct pre_form *pf,
  ...
  struct pre_form_item *new_form;

  if (!pf)
    return NULL;
  if (prev)
    new_form = prev->next = New(struct pre_form_item);
  else
    new_form = pf->item = New(struct pre_form_item);
  new_form->type = type;
  new_form->name = name;
