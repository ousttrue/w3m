...
void ISgets_to_growbuf(InputStream stream, struct growbuf *gb, char crnl) {
  ...
  while (!base->iseos) {
    ...
    growbuf_append(gb, (const char *)&sb->buf[sb->cur], i - sb->cur);
    sb->cur = i;
...
static Str ssl_check_cert_ident(X509 *x, char *hostname) {
  ...
  if (i >= 0) {
    ...
    ex = X509_get_ext(x, i);
    alt = (stack_st_GENERAL_NAME*)X509V3_EXT_d2i(ex);
    if (alt) {
      int n;
      GENERAL_NAME *gn;
      ...
      method = (X509V3_EXT_METHOD*)X509V3_EXT_get(ex);
