...
#ifdef USE_XFACE
static char *xface2xpm(char *xface)
{
    Image image;
    ImageCache *cache;
    FILE *f;
...
struct auth_param digest_auth_param[] = {
    {"realm", NULL}, {"domain", NULL}, {"nonce", NULL}, {"opaque", NULL}, {"stale", NULL}, {"algorithm", NULL}, {"qop", NULL}, {NULL, NULL}};
#endif
...
static int is_combining_char(unsigned char *ch)
{
    Lineprop ctype = get_mctype(ch);
...
void process_option(void)
{
    ...
    if (!select_is_multiple)
    {
        ...
        addSelectOption(&select_option[n_select], cur_option_value,
...
int HTMLtagproc1(struct parsed_tag *tag, struct html_feed_environ *h_env)
{
    ...
    case HTML_Q:
        ...
        {
            HTMLlineproc1((obuf->q_level & 1 ? (char *)"&lsquo;" : (char *)"&ldquo;"),
                          h_env);
            obuf->q_level += 1;
        }
        else
    ...
    case HTML_N_Q:
        ...
        {
            obuf->q_level -= 1;
            HTMLlineproc1((obuf->q_level & 1 ? (char *)"&rsquo;" : (char *)"&rdquo;"),
                          h_env);
        }
        else
            HTMLlineproc1("'", h_env);
...
#endif
    wc_ces name_charset =
        url_to_charset(NULL, &buf->currentURL, buf->document_charset);
            ...
            else
            {
                ...
                case HTML_IMG_ALT:
                    if (parsedtag_get_value(tag, ATTR_SRC, &p))
                    {
                        ...
                        {
                            buf->imarklist =
                                putHmarker(buf->imarklist, currentLn(buf), pos, iseq - 1);
                        }
                        s = NULL;
                        parsedtag_get_value(tag, ATTR_TITLE, &s);
                ...
                case HTML_INPUT_ALT:
                {
                    ...
                    if (a_select &&
                        parsedtag_get_value(tag, ATTR_SELECTNUMBER, &selectnumber))
                    {
                        if (selectnumber >= max_select)
                        {
                            ...
                            a_select = New_Reuse(Anchor *, a_select, max_select);
                        }
                    }
                ...
                case HTML_META:
                    ...
                    if (p && q && !strcasecmp(p, "refresh") && MetaRefresh)
                    {
                        ...
                        if (tmp)
                        {
                            ...
                            App::instance().setAlarmEvent(refresh_interval, AL_IMPLICIT_ONCE,
                                                          FUNCNAME_gorURL, p);
                        }
                        else if (refresh_interval > 0)
                        {
                            App::instance().setAlarmEvent(refresh_interval, AL_IMPLICIT,
            ...
            else
            {
                ...
                else
                {
                    if (mode == PC_KANJI1)
                        ...
                        is_hangul = 0;
                    if (!SimplePreserveSpace && mode == PC_KANJI1 && !is_hangul &&
                        !prev_is_hangul && obuf->pos > h_env->envs[h_env->envc].indent &&
                        Strlastchar(obuf->line) == ' ')
                    {
                        ...
                        if (obuf->line->length >= 3 && obuf->prev_ctype == PC_KANJI1 &&
                            Strlastchar(obuf->line) == ' ' &&
                            obuf->pos >= h_env->envs[h_env->envc].indent)
                        {
                            Strshrink(obuf->line, 1);
        ...
        for (i = 0; i < n_select; i++)
        {
            ...
            for (ip = select_option[i].first; ip; ip = ip->next)
            {
                s = Sprintf("<option_int value=\"%s\" label=\"%s\"%s>",
                            ...
                            html_quote(ip->label->ptr), ip->checked ? " selected" : "");
                pushTextLine(tl, newTextLine(s, 0));
            }
