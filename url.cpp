...
initMimeTypes(void)
{
	...
	return;
    UserMimeTypes = (struct table2**)New_N(struct table2 *, mimetypes_list->nitem);
...
loadURIMethods(char *filename)
{
    ...
    fseek(f, 0, 0);
    n = i;
    um = (struct table2*)New_N(struct table2, n + 1);
    i = 0;
    while (tmp = Strfgets(f), tmp->length > 0) {
	if (tmp->ptr[0] == '#')
	    continue;
...
initURIMethods(void)
{
	...
	return;
    urimethods = (struct table2**)New_N(struct table2 *, (methodmap_list->nitem + 1));
