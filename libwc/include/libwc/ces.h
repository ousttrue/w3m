...
typedef struct wc_output (*ConvFromFunc)(struct wc_option opts, const char* is, int len, wc_ces f_ces);
typedef struct wc_output (*CharConvFunc)(struct wc_option opts, wc_uchar, struct wc_status* st);

struct wc_ces_info {
    wc_ces id;
    const char* name;
    const char* desc;
    const struct wc_gset* gset;
    const wc_uchar* gset_ext;
...
extern struct wc_ces_info WcCesInfo[];
