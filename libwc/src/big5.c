...
struct wc_wchar
wc_big5_to_cs94w(struct wc_wchar cc)
{
...
struct wc_wchar
wc_cs94w_to_big5(struct wc_wchar cc)
{
...
struct wc_output wc_conv_from_big5(struct wc_option opts, const char* is, int len, wc_ces ces)
{
    ...
    for (; p < ep; p++) {
        ...
        case WC_BIG5_MBYTE1:
            if (WC_BIG5_MAP[*p] & LB)
                wtf_push(opts, &os, WC_CCS_BIG5, ((wc_uint32) * (p - 1) << 8) | *p);
            else
...
struct wc_output wc_char_conv_from_big5(struct wc_option opts, wc_uchar c, struct wc_status* st)
{
    ...
    case WC_BIG5_MBYTE1:
        if (WC_BIG5_MAP[c] & LB)
            wtf_push(opts, &os, WC_CCS_BIG5, ((wc_uint32)big5u << 8) | c);
        break;
    }
    st->state = -1;
    return os;
