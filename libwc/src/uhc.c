#include "uhc.h"
#include "conv.h"
#include "status.h"
#include "ccs.h"
#include "wtf.h"
#include "ucs.h"

#define C0 WC_UHC_MAP_C0
#define GL WC_UHC_MAP_GL
#define C1 WC_UHC_MAP_C1
#define LB WC_UHC_MAP_LB
#define UB WC_UHC_MAP_UB

// clang-format off
wc_uint8 WC_UHC_MAP[ 0x100 ] = {
    C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0,
    C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0, C0,
    GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL,
    GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL, GL,
    GL, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB,
    LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, GL, GL, GL, GL, GL,
    GL, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB,
    LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, LB, GL, GL, GL, GL, C0,

    C1, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB,
    UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB,
    UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB,
    UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB,
    UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB,
    UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB,
    UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB,
    UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, UB, C1,
};
// clang-format on

struct wc_wchar
wc_uhc_to_cs128w(struct wc_wchar cc)
{
    cc.code = WC_UHC_N(cc.code);
    if (cc.code < 0x4000)
        cc.ccs = WC_CCS_UHC_1;
    else {
        cc.ccs = WC_CCS_UHC_2;
        cc.code -= 0x4000;
    }
    cc.code = WC_N_CS128W(cc.code);
    return cc;
}

struct wc_wchar
wc_cs128w_to_uhc(struct wc_wchar cc)
{
    cc.code = WC_CS128W_N(cc.code);
    if (cc.ccs == WC_CCS_UHC_2)
        cc.code += 0x4000;
    cc.ccs = WC_CCS_UHC;
    cc.code = WC_N_UHC(cc.code);
    return cc;
}

wc_uint32
wc_uhc_to_N(wc_uint32 c)
{
    if (c <= 0xA1A0) /* 0x8141 - 0xA1A0 */
        return WC_UHC_N(c);
    if (c <= 0xA2A0) /* 0xA240 - 0xA2A0 */
        return WC_UHC_N(c) - 0x5E;
    if (c <= 0xA2E7) /* 0xA2E6 - 0xA2E7 */
        return WC_UHC_N(0xA2A0) - 0x5E + c - 0xA2E5;
    /* 0xA340 - 0xFEA0 */
    return WC_UHC_N(c) - ((c >> 8) - 0xA1) * 0x5E + 2;
}

struct wc_output wc_conv_from_uhc(struct wc_option opts, const char* is, int len, wc_ces ces)
{
    wc_uchar* sp = (wc_uchar*)is;
    wc_uchar* ep = sp + len;
    wc_uchar* p;
    int state = WC_UHC_NOSTATE;
    wc_uint32 uhc;

    for (p = sp; p < ep && *p < 0x80; p++)
        ;
    if (p == ep)
        return wc_output_from_charp(is);
    struct wc_output os = wc_output_from_size(len);
    if (p > sp)
        wc_output__cat_charp_n(&os, is, (int)(p - sp));

    for (; p < ep; p++) {
        switch (state) {
        case WC_UHC_NOSTATE:
            switch (WC_UHC_MAP[*p]) {
            case UB:
                state = WC_UHC_MBYTE1;
                break;
            case C1:
                wtf_push_unknown(opts, &os, p, 1);
                break;
            default:
                wc_output__cat_char(&os, (char)*p);
                break;
            }
            break;
        case WC_UHC_MBYTE1:
            if (WC_UHC_MAP[*p] & LB) {
                uhc = ((wc_uint32) * (p - 1) << 8) | *p;
                if (*(p - 1) >= 0xA1 && *p >= 0xA1 && uhc != 0xA2E6 && uhc != 0xA2E7)
                    wtf_push(opts, &os, WC_CCS_KS_X_1001, uhc);
                else
                    wtf_push(opts, &os, WC_CCS_UHC, uhc);
            } else
                wtf_push_unknown(opts, &os, p - 1, 2);
            state = WC_UHC_NOSTATE;
            break;
        }
    }
    switch (state) {
    case WC_UHC_MBYTE1:
        wtf_push_unknown(opts, &os, p - 1, 1);
        break;
    }
    return os;
}

void wc_push_to_uhc(struct wc_option opts, struct wc_output* os, struct wc_wchar cc, struct wc_status* st)
{
    while (1) {
        switch (cc.ccs) {
        case WC_CCS_US_ASCII:
            wc_output__cat_char(os, (char)cc.code);
            return;
        case WC_CCS_KS_X_1001:
            wc_output__cat_char(os, (char)((cc.code >> 8) | 0x80));
            wc_output__cat_char(os, (char)((cc.code & 0xff) | 0x80));
            return;
        case WC_CCS_UHC_1:
        case WC_CCS_UHC_2:
            cc = wc_cs128w_to_uhc(cc);
        case WC_CCS_UHC:
            wc_output__cat_char(os, (char)(cc.code >> 8));
            wc_output__cat_char(os, (char)(cc.code & 0xff));
            return;
        case WC_CCS_UNKNOWN_W:
            if (!opts.no_replace)
                wc_output__cat_charp(os, WC_REPLACE_W);
            return;
        case WC_CCS_UNKNOWN:
            if (!opts.no_replace)
                wc_output__cat_charp(os, WC_REPLACE);
            return;
        default:
            if (opts.ucs_conv)
                cc = wc_any_to_any_ces(opts, cc, st);
            else
                cc.ccs = WC_CCS_IS_WIDE(cc.ccs) ? WC_CCS_UNKNOWN_W : WC_CCS_UNKNOWN;
            continue;
        }
    }
}

struct wc_output wc_char_conv_from_uhc(struct wc_option opts, wc_uchar c, struct wc_status* st)
{
    static struct wc_output os;
    static wc_uchar uhcu;
    wc_uint32 uhc;

    if (st->state == -1) {
        st->state = WC_UHC_NOSTATE;
        os = wc_output_from_size(8);
    }

    switch (st->state) {
    case WC_UHC_NOSTATE:
        switch (WC_UHC_MAP[c]) {
        case UB:
            uhcu = c;
            st->state = WC_UHC_MBYTE1;
            return (struct wc_output) { 0 };
        case C1:
            break;
        default:
            wc_output__cat_char(&os, (char)c);
            break;
        }
        break;
    case WC_UHC_MBYTE1:
        if (WC_UHC_MAP[c] & LB) {
            uhc = ((wc_uint32)uhcu << 8) | c;
            if (uhcu >= 0xA1 && c >= 0xA1 && uhc != 0xA2E6 && uhc != 0xA2E7)
                wtf_push(opts, &os, WC_CCS_KS_X_1001, uhc);
            else
                wtf_push(opts, &os, WC_CCS_UHC, uhc);
        }
        break;
    }
    st->state = -1;
    return os;
}
