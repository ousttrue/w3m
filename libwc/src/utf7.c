...
static char base64_c_map[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
struct wc_output wc_conv_from_utf7(struct wc_option opts, const char* is, int len, wc_ces ces)
{
    ...
    struct wc_output os = wc_output_from_size(len + len / 3);
    if (p > sp)
    ...
    for (; p < ep; p++) {
        ...
        case WC_UTF7_PLUS:
            if (*p == WC_C_UTF7_MINUS)
                wtf_push_ucs(opts, &os, (wc_uint32)WC_C_UTF7_PLUS, &st);
        case WC_UTF7_BASE64:
            ...
            case BP: /* '+' */
                ...
                if (st.shift <= 0) {
                    st.base |= b >> (-st.shift);
                    if (st.base >= WC_C_UCS2_SURROGATE && st.base < WC_C_UCS2_SURROGATE_LOW) {
                        if (!high)
                            high = st.base;
                        else
                            high = 0; /* error */
                        ...
                        if (high)
                            wtf_push_ucs(opts, &os, wc_utf16_to_ucs(high, st.base), &st);
                        /* else; */ /* error */
                        high = 0;
                    } else if (st.base != WC_C_UCS2_BOM)
                        wtf_push_ucs(opts, &os, st.base, &st);
                    st.shift += 16;
                    st.base = 0;
                }
        ...
        case C1:
            ...
            break;
        default:
            wtf_push_ucs(opts, &os, (wc_uint32)*p, &st);
            break;
        }
    }
    return os;
...
static void
wc_push_ucs_to_utf7(struct wc_output* os, wc_uint32 ucs, struct wc_status* st)
{
    ...
    if (ucs < 0x80) {
        ...
        case CD:
            if (st->state == WC_UTF7_BASE64) {
                wc_output__cat_char(os, BASE64_C(st->base));
                wc_output__cat_char(os, WC_C_UTF7_MINUS);
        ...
        case BP:
            ...
            wc_output__cat_char(os, WC_C_UTF7_PLUS);
            wc_output__cat_char(os, WC_C_UTF7_MINUS);
            return;
        }
    }
    ...
    st->shift -= 6;
    wc_output__cat_char(os, BASE64_C((ucs >> st->shift) & 0x3f));
    st->shift -= 6;
    wc_output__cat_char(os, BASE64_C((ucs >> st->shift) & 0x3f));
    if (st->shift) {
        st->shift -= 6;
...
static int
wc_push_tag_to_utf7(struct wc_output* os, int ntag, struct wc_status* st)
{
    char* p;
...
void wc_push_to_utf7(struct wc_option opts, struct wc_output* os, struct wc_wchar cc, struct wc_status* st)
{
    ...
    while (1) {
        ...
        case WC_CCS_UCS2:
            ...
            return;
        case WC_CCS_UCS_TAG:
            if (opts.use_language_tag && wc_ucs_tag_to_tag(cc.code) != st->ntag)
        ...
        case WC_CCS_UNKNOWN:
            ...
            return;
        default:
            if (opts.ucs_conv && (cc.code = wc_any_to_ucs(opts, cc)) != WC_C_UCS4_ERROR)
                cc.ccs = WC_CCS_UCS2;
            else
                cc.ccs = WC_CCS_IS_WIDE(cc.ccs) ? WC_CCS_UNKNOWN_W : WC_CCS_UNKNOWN;
            continue;
...
struct wc_output wc_char_conv_from_utf7(struct wc_option opts, wc_uchar c, struct wc_status* st)
{
    ...
    case C1:
        break;
    default:
        wtf_push_ucs(opts, &os, (wc_uint32)c, st);
        break;
    }
    st->state = -1;
    return os;
