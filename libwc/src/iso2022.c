...
static void
wtf_push_iso2022(struct wc_option opts, struct wc_output* os, wc_ccs ccs, wc_uint32 code)
{
    switch (ccs) {
    ...
    wtf_push(opts, os, ccs, code);
}

struct wc_output wc_conv_from_iso2022(struct wc_option opts, const char* is, int len, wc_ces ces)
{
    ...
    for (; p < ep; p++) {
        ...
        case WC_ISO_NOSTATE:
            ...
            case GL:
                ...
                if (WC_CCS_IS_WIDE(gl_ccs)) {
                    ...
                    wtf_push_iso2022(opts, &os, gl_ccs, (wc_uint32)*p);
                break;
            case GR2:
            ...
            case GR:
                ...
                if (WC_CCS_IS_WIDE(gr_ccs)) {
                    ...
                    wtf_push_iso2022(opts, &os, gr_ccs, (wc_uint32)*p);
                else
                    wtf_push_unknown(opts, &os, p, 1);
                break;
            case C0:
                ...
                break;
            case C1:
                wtf_push(opts, &os, WC_CCS_C1, (wc_uint32)*p);
                break;
            case ESC:
                st.ss = 0;
        ...
        case WC_ISO_MBYTE1:
            ...
            case GL2:
                if (!(WC_CCS_TYPE(gl_ccs) & WC_CCS_A_CS96)) {
                    wc_output__cat_char(&os, (char)*q);
                    wc_output__cat_char(&os, (char)*p);
                    break;
                }
            case GL:
                wtf_push_iso2022(opts, &os, gl_ccs, ((wc_uint32)*q << 8) | *p);
                break;
            default:
                wtf_push_unknown(opts, &os, q, 2);
                break;
            }
            break;
        case WC_EUC_MBYTE1:
            ...
            case GR:
                ...
                wtf_push_iso2022(opts, &os, gr_ccs, ((wc_uint32)*q << 8) | *p);
                break;
            default:
                wtf_push_unknown(opts, &os, q, 2);
                break;
        ...
        case WC_EUC_TW_MBYTE2:
            if (WC_ISO_MAP[*p] == GR) {
                if (0xa1 <= *q && *q <= 0xa7) {
                    wtf_push_iso2022(opts, &os, WC_CCS_CNS_11643_1 + (*q - 0xa1),
                        ((wc_uint32) * (q + 1) << 8) | *p);
                    break;
                }
                if (0xa8 <= *q && *q <= 0xb0) {
                    wtf_push_iso2022(opts, &os, WC_CCS_CNS_11643_8 + (*q - 0xa8),
                        ((wc_uint32) * (q + 1) << 8) | *p);
                    break;
                }
            }
            wtf_push_unknown(opts, &os, q, 3);
            break;
        case WC_ISO_CSWSR:
            ...
            continue;
        case WC_ISO_CSWOSR:
            wtf_push_unknown(opts, &os, p, ep - p);
            return os;
            break;
        }
        st.ss = 0;
...
int wc_parse_iso2022_esc(wc_uchar** ptr, struct wc_status* st)
{
...
void wc_push_to_iso2022(struct wc_option opts, struct wc_output* os, struct wc_wchar cc, struct wc_status* st)
{
    wc_uchar g = 0;
    ...
    while (1) {
        ...
        case WC_CCS_A_CS94W:
            ...
            switch (cc.ccs) {
            case WC_CCS_JIS_X_0212:
                if (!opts.use_jisx0212 && opts.use_jisx0213 && opts.ucs_conv) {
                    cc2 = wc_jisx0212_to_jisx0213(opts, cc);
                    if (cc2.ccs == WC_CCS_JIS_X_0213_1 || cc2.ccs == WC_CCS_JIS_X_0213_2) {
                        cc = cc2;
                        continue;
                    }
                }
            ...
            case WC_CCS_JIS_X_0213_1:
            case WC_CCS_JIS_X_0213_2:
                if (!opts.use_jisx0213 && opts.use_jisx0212 && opts.ucs_conv) {
                    cc2 = wc_jisx0213_to_jisx0212(opts, cc);
        ...
        case WC_CCS_A_UNKNOWN:
            ...
            break;
        default:
            if ((cc.ccs == WC_CCS_JOHAB || cc.ccs == WC_CCS_JOHAB_1 || cc.ccs == WC_CCS_JOHAB_2 || cc.ccs == WC_CCS_JOHAB_3) && cs94w_gmap[WC_F_KS_X_1001 - WC_F_ISO_BASE]) {
                struct wc_wchar cc2 = wc_johab_to_ksx1001(opts, cc);
            ...
            if (opts.ucs_conv)
                cc = wc_any_to_iso2022(opts, cc, st);
            else
                cc.ccs = WC_CCS_IS_WIDE(cc.ccs) ? WC_CCS_UNKNOWN_W : WC_CCS_UNKNOWN;
            continue;
        }
        if (!g) {
            if (opts.ucs_conv && !retry)
...
void wc_push_to_iso2022_end(struct wc_output* os, struct wc_status* st)
{
...
void wc_push_iso2022_esc(struct wc_output* os, wc_ccs ccs, wc_uchar g, wc_uint8 invoke, struct wc_status* st)
{
    wc_uint8 g_invoke = g & 0x03;

    if (st->design[g_invoke] != ccs) {
        wc_output__cat_char(os, WC_C_ESC);
        if (WC_CCS_IS_WIDE(ccs)) {
            wc_output__cat_char(os, WC_C_MBCS);
            if (g_invoke != 0 || (ccs != WC_CCS_JIS_C_6226 && ccs != WC_CCS_JIS_X_0208 && ccs != WC_CCS_GB_2312))
                wc_output__cat_char(os, (char)g);
        } else {
            wc_output__cat_char(os, (char)g);
            if ((ccs & WC_CCS_A_ISO_2022) == WC_CCS_A_CS942)
                wc_output__cat_char(os, WC_C_CS942);
        }
        wc_output__cat_char(os, (char)WC_CCS_GET_F(ccs));
    ...
    case 0:
        if (st->gl != 0) {
            wc_output__cat_char(os, WC_C_SI);
            st->gl = 0;
        }
        break;
    case 1:
        if (st->gl != 1) {
            wc_output__cat_char(os, WC_C_SO);
            st->gl = 1;
        }
        break;
    case 2:
        wc_output__cat_char(os, WC_C_ESC);
        wc_output__cat_char(os, WC_C_SS2);
        break;
    case 3:
        wc_output__cat_char(os, WC_C_ESC);
        wc_output__cat_char(os, WC_C_SS3);
        break;
    }
}

void wc_push_to_euc(struct wc_option opts, struct wc_output* os, struct wc_wchar cc, struct wc_status* st)
{
    ...
    while (1) {
        ...
        case WC_CCS_JOHAB_3:
            if (st->ces_info->id == WC_CES_EUC_KR) {
                cc = wc_johab_to_ksx1001(opts, cc);
                continue;
            }
        default:
            if (opts.ucs_conv)
...
void wc_push_to_eucjp(struct wc_option opts, struct wc_output* os, struct wc_wchar cc, struct wc_status* st)
{
    while (1) {
        switch (cc.ccs) {
        case WC_CCS_US_ASCII:
        ...
        case WC_CCS_JIS_X_0201K:
            if (opts.use_jisx0201k) {
                wc_output__cat_char(os, WC_C_SS2R);
                wc_output__cat_char(os, (char)(cc.code | 0x80));
                return;
        ...
        case WC_CCS_JIS_X_0213_1:
            if (opts.use_jisx0213)
                break;
            else if (opts.ucs_conv && opts.use_jisx0212)
                cc = wc_jisx0213_to_jisx0212(opts, cc);
            else
                cc.ccs = WC_CCS_UNKNOWN_W;
            continue;
        case WC_CCS_JIS_X_0212:
            if (opts.use_jisx0212) {
                wc_output__cat_char(os, WC_C_SS3R);
                break;
            } else if (opts.ucs_conv && opts.use_jisx0213)
                cc = wc_jisx0212_to_jisx0213(opts, cc);
            else
                cc.ccs = WC_CCS_UNKNOWN_W;
            continue;
        case WC_CCS_JIS_X_0213_2:
...
void wc_push_to_euctw(struct wc_option opts, struct wc_output* os, struct wc_wchar cc, struct wc_status* st)
{
    while (1) {
        switch (cc.ccs) {
        case WC_CCS_US_ASCII:
        ...
        case WC_CCS_CNS_11643_7:
            wc_output__cat_char(os, WC_C_SS2R);
            wc_output__cat_char(os, (char)(0xA1 + (cc.ccs - WC_CCS_CNS_11643_1)));
            break;
        ...
        case WC_CCS_CNS_11643_16:
            wc_output__cat_char(os, WC_C_SS2R);
            wc_output__cat_char(os, (char)(0xA8 + (cc.ccs - WC_CCS_CNS_11643_8)));
            break;
        case WC_CCS_C1:
            wc_output__cat_char(os, (char)(cc.code | 0x80));
            return;
...
void wc_push_to_iso8859(struct wc_option opts, struct wc_output* os, struct wc_wchar cc, struct wc_status* st)
{
    wc_ccs g1_ccs = st->ces_info->gset[1].ccs;
...
void wc_create_gmap(struct wc_option opts, struct wc_status* st)
{
    const struct wc_gset* gset = st->ces_info->gset;
    const wc_uchar* gset_ext = st->ces_info->gset_ext;
    int i, f;

    if (opts.strict_iso2022) {
...
struct wc_output wc_char_conv_from_iso2022(struct wc_option opts, wc_uchar c, struct wc_status* st)
{
    ...
    case WC_ISO_NOSTATE:
        ...
        case GL:
            if (WC_CCS_IS_WIDE(gl_ccs)) {
                ...
                wtf_push_iso2022(opts, &os, gl_ccs, (wc_uint32)c);
            break;
        case GR2:
            if (!(WC_CCS_TYPE(gr_ccs) & WC_CCS_A_CS96))
                break;
        case GR:
            if (WC_CCS_IS_WIDE(gr_ccs)) {
                ...
                wtf_push_iso2022(opts, &os, gr_ccs, (wc_uint32)c);
            break;
        case C0:
            wc_output__cat_char(&os, (char)c);
            break;
    ...
    case WC_ISO_MBYTE1:
        ...
        case GL:
            buf[nbuf++] = c;
            wtf_push_iso2022(opts, &os, gl_ccs, ((wc_uint32)buf[0] << 8) | buf[1]);
            break;
        }
        st->state = WC_ISO_NOSTATE;
        break;
    case WC_EUC_MBYTE1:
        ...
        case GR:
            ...
            buf[nbuf++] = c;
            wtf_push_iso2022(opts, &os, gr_ccs, ((wc_uint32)buf[0] << 8) | buf[1]);
            break;
        }
        st->state = WC_ISO_NOSTATE;
        break;
    case WC_EUC_TW_MBYTE2:
        if (WC_ISO_MAP[c] == GR) {
            ...
            c = buf[0];
            if (0xa1 <= c && c <= 0xa7) {
                wtf_push_iso2022(opts, &os, WC_CCS_CNS_11643_1 + (c - 0xa1),
                    ((wc_uint32)buf[1] << 8) | buf[2]);
                break;
            }
            if (0xa8 <= c && c <= 0xb0) {
                wtf_push_iso2022(opts, &os, WC_CCS_CNS_11643_8 + (c - 0xa8),
                    ((wc_uint32)buf[1] << 8) | buf[2]);
                break;
            }
        }
