...
static struct wc_map ucs_jisx0208_map[ N_ucs_jisx0208_map ] = {
EOF
for(@ucs) {
  $_ || next;
...
static struct wc_map ucs_jisx0212_map[ N_ucs_jisx0212_map ] = {
EOF
for(@ucs) {
  $_ || next;
  printf OUT "  { 0x%.4X, 0x%.4X },\n", $_, $from_ucs2{$_};
...
static struct wc_map ucs_jisx02131_map[ N_ucs_jisx02131_map ] = {
EOF
for(@ucs) {
  $_ || next;
  printf OUT "  { 0x%.4X, 0x%.4X },\n", $_, $from_ucs3{$_};
...
static struct wc_map ucs_jisx02132_map[ N_ucs_jisx02132_map ] = {
EOF
for(@ucs) {
  $_ || next;
  printf OUT "  { 0x%.4X, 0x%.4X },\n", $_, $from_ucs4{$_};
