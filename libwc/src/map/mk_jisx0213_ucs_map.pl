...
static struct wc_map ${name}1_ucs_p2_map[ N_${name}1_ucs_p2_map ] = {
EOF
for(@ucs) {
  $_ || next;
...
static struct wc_map ucs_p2_${name}1_map[ N_ucs_p2_${name}1_map ] = {
EOF
for(@ucs) {
  $_ || next;
  printf OUT "  { 0x%.4X, 0x%.4X },\n", $_, $from_ucs1{$_};
...
static struct wc_map ${name}2_ucs_p2_map[ N_${name}2_ucs_p2_map ] = {
EOF
for(@ucs) {
  $_ || next;
  printf OUT "  { 0x%.4X, 0x%.4X },\n", $_, $to_ucs2{$_};
...
static struct wc_map ucs_p2_${name}2_map[ N_ucs_p2_${name}2_map ] = {
EOF
for(@ucs) {
  $_ || next;
  printf OUT "  { 0x%.4X, 0x%.4X },\n", $_, $from_ucs2{$_};
