...
struct wc_wchar
wc_ucs_to_any(wc_uint32 ucs, struct wc_table* t)
{
    struct wc_wchar cc;
    struct wc_map* map;
    ...
    if (t && (ucs & ~0xFFFF) == WC_C_UCS4_PLANE2) {
        ...
        else if (t->ccs == WC_CCS_HKSCS || t->ccs == WC_CCS_HKSCS_1 || t->ccs == WC_CCS_HKSCS_2)
            map = wc_map_search((wc_uint16)(ucs & 0xffff),
...
wc_uint32
wc_any_to_ucs(struct wc_option opts, struct wc_wchar cc)
{
    int f;
    wc_uint16* map = NULL;
    ...
    case WC_CCS_A_CS94:
        ...
        break;
    case WC_CCS_A_CS94W:
        if (cc.ccs == WC_CCS_GB_2312 && opts.use_gb12345_map) {
    ...
    case WC_CCS_A_PCS:
        ...
        case WC_CCS_TCVN_5712_3:
            return wc_any_to_ucs(opts, wc_tcvn57123_to_tcvn5712(cc));
        case WC_CCS_GBK_80:
    ...
    case WC_CCS_A_PCSW:
        ...
        case WC_CCS_HKSCS:
            ...
            break;
        case WC_CCS_JOHAB:
            return wc_any_to_ucs(opts, wc_johab_to_cs128w(cc));
        case WC_CCS_JOHAB_1:
        ...
        case WC_CCS_SJIS_EXT:
            return wc_any_to_ucs(opts, wc_sjis_ext_to_cs94w(cc));
        ...
        case WC_CCS_GBK_EXT_2:
            return wc_gb18030_to_ucs(opts, cc);
        case WC_CCS_UHC_1:
        case WC_CCS_UHC_2:
...
struct wc_wchar
wc_any_to_any(struct wc_option opts, struct wc_wchar cc, struct wc_table* t)
{
    wc_ccs is_wide = WC_CCS_IS_WIDE(cc.ccs);
    wc_uint32 ucs = wc_any_to_ucs(opts, cc);
...
struct wc_wchar
wc_ucs_to_any_list(wc_uint32 ucs, struct wc_table** tlist)
{
    struct wc_wchar cc;
    struct wc_table** t;
...
struct wc_wchar
wc_any_to_any_ces(struct wc_option opts, struct wc_wchar cc, struct wc_status* st)
{
    wc_uint32 ucs = wc_any_to_ucs(opts, cc);
    wc_ccs is_wide = WC_CCS_IS_WIDE(cc.ccs);
    ...
    if (ucs != WC_C_UCS4_ERROR) {
        ...
        if (st->ces_info->id & (WC_CES_T_ISO_8859 | WC_CES_T_EUC) && 0x80 <= ucs && ucs <= 0x9F) {
            cc.ccs = WC_CCS_C1;
            cc.code = ucs;
            return cc;
        }
...
struct wc_wchar
wc_any_to_iso2022(struct wc_option opts, struct wc_wchar cc, struct wc_status* st)
{
    wc_uint32 ucs = wc_any_to_ucs(opts, cc);
    wc_ccs is_wide = WC_CCS_IS_WIDE(cc.ccs);
...
wc_ccs
wc_ucs_to_ccs(struct wc_option opts, wc_uint32 ucs)
{
    if (0x80 <= ucs && ucs <= 0x9F)
        return WC_CCS_C1;
    return ((ucs <= WC_C_UCS2_END) ? WC_CCS_UCS2 : WC_CCS_UCS4)
        | ((opts.east_asian_width && wc_is_ucs_ambiguous_width(opts, ucs))
                ? WC_CCS_A_WIDE
                : 0)
        | (wc_is_ucs_wide(ucs) ? WC_CCS_A_WIDE : 0)
        | (wc_is_ucs_combining(opts, ucs) ? WC_CCS_A_COMB : 0);
}

wc_bool
wc_is_ucs_ambiguous_width(struct wc_option opts, wc_uint32 ucs)
{
    if (0xa1 <= ucs && ucs <= 0xfe && opts.use_jisx0213)
        return 1;
    else if (ucs <= WC_C_UCS2_END)
        return (wc_map_range_search((wc_uint16)ucs,
                    ucs_ambwidth_map, N_ucs_ambwidth_map)
            != NULL);
    else
...
wc_is_ucs_wide(wc_uint32 ucs)
{
    if (ucs <= WC_C_UCS2_END)
        return (wc_map_range_search((wc_uint16)ucs,
                    ucs_wide_map, N_ucs_wide_map)
            != NULL);
    else
        return ((ucs & ~0xFFFF) == WC_C_UCS4_PLANE2 || (ucs & ~0xFFFF) == WC_C_UCS4_PLANE3);
}

wc_bool
wc_is_ucs_combining(struct wc_option opts, wc_uint32 ucs)
{
    return (opts.use_combining && ucs <= WC_C_UCS2_END && wc_map_range_search((wc_uint16)ucs, ucs_combining_map, N_ucs_combining_map) != NULL);
}

wc_bool
wc_is_ucs_hangul(wc_uint32 ucs)
{
    return (ucs <= WC_C_UCS2_END && wc_map_range_search((wc_uint16)ucs, ucs_hangul_map, N_ucs_hangul_map) != NULL);
}

wc_bool
wc_is_ucs_alpha(wc_uint32 ucs)
{
    return (ucs <= WC_C_UCS2_END && wc_map_range_search((wc_uint16)ucs, ucs_isalpha_map, N_ucs_isalpha_map) != NULL);
}

wc_bool
wc_is_ucs_digit(wc_uint32 ucs)
{
    return (ucs <= WC_C_UCS2_END && wc_map_range_search((wc_uint16)ucs, ucs_isdigit_map, N_ucs_isdigit_map) != NULL);
}
...
{
    return (ucs <= WC_C_UCS2_END && wc_map_range_search((wc_uint16)ucs, ucs_islower_map, N_ucs_islower_map) != NULL);
}

wc_bool
wc_is_ucs_upper(wc_uint32 ucs)
{
    return (ucs <= WC_C_UCS2_END && wc_map_range_search((wc_uint16)ucs, ucs_isupper_map, N_ucs_isupper_map) != NULL);
}

wc_uint32
wc_ucs_toupper(wc_uint32 ucs)
{
    struct wc_map* conv = NULL;
    if (ucs <= WC_C_UCS2_END)
...
wc_uint32
wc_ucs_precompose(struct wc_option opts, wc_uint32 ucs1, wc_uint32 ucs2)
{
    struct wc_map3* map;

    if (opts.use_combining && ucs1 <= WC_C_UCS2_END && ucs2 <= WC_C_UCS2_END && (map = wc_map3_search((wc_uint16)ucs1, (wc_uint16)ucs2, ucs_precompose_map, N_ucs_precompose_map)) != NULL)
        return map->code3;
    return WC_C_UCS4_ERROR;
}
...
wc_ucs_to_fullwidth(wc_uint32 ucs)
{
    ...
    if (ucs <= WC_C_UCS2_END && (map = wc_map_search((wc_uint16)ucs, ucs_fullwidth_map, N_ucs_fullwidth_map)) != NULL)
        return map->code2;
    return WC_C_UCS4_ERROR;
}
...
void wtf_push_ucs(struct wc_option opts, struct wc_output* os, wc_uint32 ucs, struct wc_status* st)
{
    wc_ccs ccs;

    if (ucs >= WC_C_LANGUAGE_TAG0 && ucs <= WC_C_CANCEL_TAG) {
        if (!opts.use_language_tag)
            return;
        if (ucs == WC_C_LANGUAGE_TAG)
            // if (st->tag)
            //     Strclear(st->tag);
            // else
            st->tag = wc_output_from_size(MAX_TAG_LEN);
        else if (ucs == WC_C_CANCEL_TAG) {
            // if (st->tag)
            wc_output__free(&st->tag);
            st->tag = (struct wc_output) { 0 };
            st->ntag = 0;
        } else if (st->tag.data && wc_output__len(st->tag) < MAX_TAG_LEN && ucs >= WC_C_TAG_SPACE)
            wc_output__cat_char(&st->tag, (char)(ucs & 0x7f));
        return;
    }
    if (st->tag.data) {
        st->ntag = wc_ucs_put_tag(wc_output__ptr(st->tag));
        st->tag = (struct wc_output) { 0 };
    }
    if (ucs < 0x80) {
        if (st->ntag)
            wtf_push(opts, os, WC_CCS_UCS_TAG, wc_ucs_to_ucs_tag(ucs, st->ntag));
        else
            wc_output__cat_char(os, (char)ucs);
    } else {
        ccs = wc_ucs_to_ccs(opts, ucs);
        if (st->ntag && ucs <= WC_C_UNICODE_END) {
            ccs = wc_ccs_ucs_to_ccs_ucs_tag(ccs);
            ucs = wc_ucs_to_ucs_tag(ucs, st->ntag);
        }
        wtf_push(opts, os, ccs, ucs);
    }
}
