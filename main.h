...
struct Url;
struct form_list;
void cmd_loadURL(const char* url, struct Url* current, char* referer, struct form_list* request);
void _movL(int n);
void _movD(int n);
void _movU(int n);
void _movR(int n);
...
void _newT(void);
struct Buffer* loadLink(const char* url, const char* target, const char* referer, struct form_list* request);
