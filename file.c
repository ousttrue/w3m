    ...
    struct input_stream* connection)
{
    ...
    struct Content content = openURL(url, current, request, option, connection);
    if (!content.stream && getRuntime()->retryAsHttp && content.url_str[0] != '/') {
        if (content.url.scheme == SCM_MISSING || content.url.scheme == SCM_UNKNOWN) {
            // retry it as "http://"
            const char* u = Strnew_m_charp("http://", path, NULL)->ptr;
            parseURL2(u, &url, current);
            content = openURL(url, current, request, option, connection);
        }
    }
    ...
    if (!content.stream) {
        ...
        case SCM_LOCAL: {
            struct stat st;
            if (stat(content.url.real_file, &st) < 0)
                return NULL;
            if (S_ISDIR(st.st_mode)) {
                if (getRuntime()->UseExternalDirBuffer) {
                    Str cmd = Sprintf("%s?dir=%s#current",
                        getRuntime()->DirBufferCommand, content.url.file);
    ...
    if (content.status == HTST_MISSING) {
        TRAP_OFF;
        is_close(content.stream);
        return NULL;
    }
    ...
    if (content.url.scheme == SCM_HTTP || content.url.scheme == SCM_HTTPS || (((content.url.scheme == SCM_FTP && non_null(getRuntime()->FTP_proxy))) && getRuntime()->use_proxy && !check_no_proxy(content.url.host))) {
        ...
        if (auth.add_auth_cookie_flag
            ...
            add_auth_user_passwd(&content.url, qstr_unquote(auth.realm)->ptr,
                auth.uname,
                auth.pwd,
                0);
        ...
        if ((p = checkHeader(&content, "WWW-Authenticate:")) != NULL //
            ...
            if (findAuthentication(&hauth, content.document_header, "WWW-Authenticate:") != NULL
                ...
                getAuthCookie(&hauth, "Authorization:", option.extra_header,
                    auth_pu, &content.hr, request, &auth.uname, &auth.pwd);
                if (auth.uname == NULL) {
                    /* abort */
                    TRAP_OFF;
        ...
        if ((p = checkHeader(&content, "Proxy-Authenticate:")) != NULL //
            ...
            if (findAuthentication(&hauth, content.document_header, "Proxy-Authenticate:")
                ...
                getAuthCookie(&hauth, "Proxy-Authorization:",
                    option.extra_header, auth_pu, &content.hr, request,
                    &auth.uname, &auth.pwd);
                if (auth.uname == NULL) {
                    /* abort */
                    TRAP_OFF;
...
Str process_anchor(struct HtmlBuilder* hb,
    struct HtmlTag* tag, const char* tagbuf)
{
...
int HTMLtagproc1(struct HtmlBuilder* hb, struct HtmlTag* tag, struct html_feed_environ* h_env)
{
    const char* p;
    const char* q;
    const char* r;
        ...
        while (str < endp) {
            ...
            if ((effect | ex_efct(ex_effect)) & PC_SYMBOL && *str != '<') {
                ...
                case HTML_A:
                    ...
                    if (hseq > 0)
                        buf->doc.hmarklist = putHmarker(buf->doc.hmarklist, currentLn(buf),
                            pos, hseq - 1);
                    else if (hseq < 0) {
                        int h = -hseq - 1;
                        if (buf->doc.hmarklist && h < buf->doc.hmarklist->nmark && buf->doc.hmarklist->marks[h].invalid) {
                            buf->doc.hmarklist->marks[h].pos = pos;
                            buf->doc.hmarklist->marks[h].line = currentLn(buf);
                            buf->doc.hmarklist->marks[h].invalid = 0;
                            hseq = -hseq;
                        }
                    }
                    if (id && idFrame)
                ...
                case HTML_N_A:
                    ...
                    if (a_href) {
                        ...
                        if (a_href->start.line == a_href->end.line && a_href->start.pos == a_href->end.pos) {
                            if (buf->doc.hmarklist && a_href->hseq >= 0 && a_href->hseq < buf->doc.hmarklist->nmark)
                                buf->doc.hmarklist->marks[a_href->hseq].invalid = 1;
                            a_href->hseq = -1;
                        }
                        a_href = NULL;
                    }
                ...
                case HTML_IMG_ALT:
                    if (parsedtag_get_value(tag, ATTR_SRC, &p)) {
                        ...
                        if (iseq > 0) {
                            buf->doc.imarklist = putHmarker(buf->doc.imarklist,
                                currentLn(buf), pos,
                                iseq - 1);
                        }
                        ...
                        if (iseq > 0) {
                            ...
                            struct BufferPoint* po = buf->doc.imarklist->marks - iseq - 1;
                ...
                case HTML_INPUT_ALT: {
                    ...
                    if (hseq > 0) {
                        ...
                        if (*str == '[')
                            hpos++;
                        if (buf->doc.hmarklist && h < buf->doc.hmarklist->nmark && buf->doc.hmarklist->marks[h].invalid) {
                            buf->doc.hmarklist->marks[h].pos = hpos;
                            buf->doc.hmarklist->marks[h].line = currentLn(buf);
                            buf->doc.hmarklist->marks[h].invalid = 0;
                            hseq = -hseq;
                        }
                ...
                case HTML_N_MAP:
                    ...
                    break;
                case HTML_AREA:
                    if (buf->doc.maplist == NULL) /* outside of <map>..</map> */
                        break;
                    if (parsedtag_get_value(tag, ATTR_HREF, &p)) {
                        struct MapArea* a;
                        p = url_encode(remove_space(p), base,
                        ...
                        pushValue(buf->doc.maplist->area, (void*)a);
                    }
                    break;
                case HTML_FRAMESET:
                    frameset_sp++;
