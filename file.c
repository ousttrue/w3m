...
static void addLink(struct Buffer* buf, struct HtmlTag* tag);
...
Str loadGopherDir(struct URLFile* uf, struct Url* pu, wc_ces* charset)
{
    ...
    pre = 0;
    while (1) {
        if (!(lbuf = StrISgets(uf->stream)) || lbuf->length == 0)
            break;
...
int save2tmp(InputStream stream, enum UrlScheme scheme, const char* tmpf)
{
    FILE* ff;
    int64_t linelen = 0, trbyte = 0;
    SignalFunc prevtrap = NULL;
    ...
    if (scheme == SCM_NEWS) {
        ...
        if (!stream)
            return -1;
        while (c = ISgetc(stream), !iseos(stream)) {
            if (c == '\n') {
                if (check == 0)
                    check++;
                else if (check == 3)
