...
static void KeyAbort(SIGNAL_ARG) { LONGJMP(AbortLoading, 1); }

static int parseFrameSetLength(char *s, char ***ret) {
  int i, len;
  char *p, *q, **lv;
...
struct frameset *newFrameSet(struct parsed_tag *tag) {
  ...
  f = (struct frameset *)New(struct frameset);
  f->attr = F_FRAMESET;
  f->name = NULL;
  f->currentURL = NULL;
  ...
  f->i = 0;
  i = f->row * f->col;
  f->frame = (union frameset_element *)New_N(union frameset_element, i);
  do {
...
struct frame_body *newFrame(struct parsed_tag *tag, Buffer *buf) {
  ...
  body = (struct frame_body *)New(struct frame_body);
  ...
  if (tag) {
    if (parsedtag_get_value(tag, ATTR_SRC, &p))
      body->url =
          url_encode(remove_space(p), body->baseURL, buf->document_charset);
...
static void unloadFrame(struct frame_body *b) { b->attr = F_UNLOADED; }
...
static struct frame_body *copyFrame(struct frame_body *ob) {
  struct frame_body *rb;

  rb = (struct frame_body *)New(struct frame_body);
...
struct frameset *copyFrameSet(struct frameset *of) {
  ...
  rf = (struct frameset *)New(struct frameset);
  n = of->col * of->row;
  bcopy((const void *)of, (void *)rf, sizeof(struct frameset));
  rf->width = (char **)New_N(char *, rf->col);
  bcopy((const void *)of->width, (void *)rf->width, sizeof(char *) * rf->col);
  rf->height = (char **)New_N(char *, rf->row);
  bcopy((const void *)of->height, (void *)rf->height, sizeof(char *) * rf->row);
  rf->frame = (union frameset_element *)New_N(union frameset_element, n);
  while (n) {
    n--;
...
static void flushFrameSet(struct frameset *fs) {
  int n = fs->i;
...
void pushFrameTree(struct frameset_queue **fqpp, struct frameset *fs,
  ...
  rfq = (struct frameset_queue *)New(struct frameset_queue);
  rfq->linenumber =
      (buf && buf->currentLine) ? buf->currentLine->linenumber : 1;
...
void resetFrameElement(union frameset_element *f_element, Buffer *buf,
  ...
  if (buf->frameset) {
    ...
    f_element->set->currentURL = (ParsedURL *)New(ParsedURL);
...
static struct frameset *frame_download_source(struct frame_body *b,
  ...
  default:
    is_redisplay = TRUE;
    w3m_dump |= DUMP_FRAME;
    buf = loadGeneralFile(b->url, baseURL ? baseURL : currentURL, b->referer,
                          flag | RG_FRAME_SRC, b->request);
    /* XXX certificate? */
  ...
  if (buf->frameset) {
    ...
    ret_frameset->currentURL = (ParsedURL *)New(ParsedURL);
  ...
  for (r = 0; r < f->row; r++) {
    ...
    for (c = 0; c < f->col; c++) {
      ...
      case F_UNLOADED:
        ...
        fflush(f1);
        f_frameset = frame_download_source(frame.body, currentURL,
      ...
      case F_BODY:
        ...
        if (f2.stream == NULL) {
          ...
          if (frame.body->flags & FB_NO_BUFFER)
            ...
            /* FIXME: gettextize? */
            fprintf(f1, "Can't open %s", html_quote(frame.body->url));
          else
            /* FIXME: gettextize? */
          ...
          if (tok->ptr[0] == '<') {
            if (tok->ptr[1] && REALLY_THE_BEGINNING_OF_A_TAG(tok->ptr))
              is_tag = TRUE;
                       ...
                       (RB_PLAIN | RB_INTXTA | RB_SCRIPT | RB_STYLE))) {
          ...
          if (is_tag) {
            ...
            case HTML_N_TABLE:
              ...
              if (t_stack < 0) {
                ...
                Strshrinkfirst(tok, 1);
                Strshrink(tok, 1);
                fprintf(f1, "<!-- table stack underflow: %s -->",
                        html_quote(tok->ptr));
                goto token_end;
              }
              break;
            ...
            for (j = 0; j < TagMAP[tag->tagid].max_attribute; j++) {
              ...
              case ATTR_ACTION:
                if (!tag->value[j])
                  break;
                tag->value[j] =
                    url_encode(remove_space(tag->value[j]), &base, charset);
                ...
                parsedtag_set_value(tag, ATTR_REFERER,
              ...
              case ATTR_TARGET:
                ...
                if (!strcasecmp(tag->value[j], "_self")) {
                  parsedtag_set_value(tag, ATTR_TARGET, s_target);
                } else if (!strcasecmp(tag->value[j], "_parent")) {
                  parsedtag_set_value(tag, ATTR_TARGET, p_target);
                }
                break;
              case ATTR_NAME:
              case ATTR_ID:
                if (!tag->value[j])
                  break;
                parsedtag_set_value(tag, ATTR_FRAMENAME, s_target);
                break;
              }
            }
            if (a_target == 1) {
            ...
            if (pre_mode & RB_PLAIN)
              ...
              fprintf(f1, "%s", html_quote(html_unquote(tok->ptr)));
            else
              Strfputs(tok, f1);
          }
        token_end:
